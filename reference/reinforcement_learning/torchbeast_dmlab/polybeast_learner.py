...
parser.add_argument("--seed", default=1, type=int,
                    help="seed for training. not used here.")
parser.add_argument("--valid_seed", default=42, type=int,
                    help="seed for validation env.")
...
parser.add_argument("--validate_every", default=240, type=int,
...
parser.add_argument("--disable_cuda", action="store_true",
                    help="Disable CUDA.")
parser.add_argument("--num_actions", default=9, type=int, metavar="A",
                    help="Number of actions. 6 for Atari, 9 for DMLab")
...
def train(flags):
    ...
    if flags.use_quasi_lstm:
        ...
        actor_model = Net(num_actions=flags.num_actions,
                          hidden_size=flags.hidden_size,
                          freeze_conv=flags.freeze_conv, freeze_fc=flags.freeze_fc,
                          use_lstm=flags.use_lstm, is_dmlab=True)
        actor_model.to(device=flags.actor_device)
    ...
    def validate(num_episodes=5, num_runs=1, plogger=None, step=0):
        ...
        from torchbeast_dmlab.polybeast_env import create_test_env

        env_name = flags.env
        gym_env = create_test_env(env_name, flags.valid_seed)
    ...
    full_test(flags, plogger)
    logging.info(f'End of training after {stats["step"]} steps.')


def full_test(flags, plogger, num_episodes=100, num_runs=3, device='cuda'):
    import numpy as np
    ...
    for run_id in range(num_runs):
        ...
        # changing seed between eval sets; but this should not really matter.
        gym_env = create_test_env(flags.env, run_id)
        env = environment.Environment(gym_env)
        ...
        plogger._logger.info(f"Start eval {num_episodes} episodes from seed {run_id}")
        with torch.no_grad():
            returns = []
            while len(returns) < num_episodes:
                ...
                if observation["done"].item():
                    ...
                    plogger._logger.info(
                        f'[Episode {ep_counter} ended after {observation["episode_step"].item()} steps. Return: {observation["episode_return"].item() :.1f}',
                    )
        env.close()
...
def test(flags, num_episodes=30, num_runs=5, device='cuda'):
    ...
    gym_env = create_test_env(flags.env, flags.valid_seed)
    env = environment.Environment(gym_env)
