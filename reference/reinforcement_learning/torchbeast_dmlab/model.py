...
from torchbeast.layer import QuasiLSTMlayer, RTRLQuasiLSTMlayer, SnapQuasiLSTMlayer, QuasiFullLSTMlayer


# Baseline model from torchbeast
class Net(nn.Module):
    def __init__(self, num_actions, use_lstm=False, is_dmlab=False,
        ...
        if is_dmlab:
            self.vision_out_dim = 3456
        else:
            self.vision_out_dim = 3872
        ...
        self.fc = nn.Linear(self.vision_out_dim, 256)
        ...
        if use_lstm:
            self.core = nn.LSTM(core_output_size, hidden_size, num_layers=num_lstm_layers)
...
class QuasiFullLSTMNet(nn.Module):
    def __init__(self, num_actions, hidden_size=256,
                 freeze_conv=False, freeze_fc=False, is_dmlab=False):
        super().__init__()
        ...
        scaled_num_ch = [c for c in base_num_ch]
        ...
        self.core = QuasiFullLSTMlayer(core_output_size, hidden_size)
        core_output_size = hidden_size
...
class SnapQuasiLSTMNet(nn.Module):
    def __init__(self, num_actions, hidden_size=256,
                 freeze_conv=False, freeze_fc=False, is_dmlab=False):
        super().__init__()
        ...
        self.core = SnapQuasiLSTMlayer(self.rnn_input_dim, hidden_size)
        core_output_size = hidden_size
    ...
    def initial_state(self, batch_size=1):
        return (
            ...
            (torch.zeros(1, batch_size, self.hidden_size, self.rnn_input_dim),
             torch.zeros(1, batch_size, self.hidden_size, self.rnn_input_dim),
             torch.zeros(1, batch_size, self.hidden_size, self.hidden_size),
             torch.zeros(1, batch_size, self.hidden_size, self.hidden_size),
             torch.zeros(1, batch_size, self.hidden_size),
             torch.zeros(1, batch_size, self.hidden_size),)
        )
...
#            if self.use_xem:
#                # average across all finished episode?
#                # but it might not be likely to have many finished states
#                # at the same time?
#                assert False, "wip"
