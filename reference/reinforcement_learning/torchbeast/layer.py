...
class SnapQuasiLSTMlayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, forget_bias=0.):
        super().__init__()
        ...
        # weight vectors
        self.wv_z = nn.Parameter(
            torch.rand(hidden_dim, hidden_dim), requires_grad=False)
        self.wv_f = nn.Parameter(
            torch.rand(hidden_dim, hidden_dim), requires_grad=False)
        # self.wv_z = nn.Parameter(
        #     torch.rand(1, hidden_dim), requires_grad=False)  # append B dim
    ...
    def forward_actor(self, x, state=None):
        ...
        for x_, pre_z_, pre_f_ in zip(x, out_z, out_f):
            z_part = torch.tanh(
                pre_z_ + F.linear(new_cell, self.wv_z) + self.bias_z)
            f_part = torch.sigmoid(
                pre_f_ + F.linear(new_cell, self.wv_f) + self.bias_f)
            ...
            with torch.no_grad():
                ...
                zf_tmp = (1. - f_part) * (1. - z_part ** 2)
                fz_tmp = (new_cell - z_part) * (1. - f_part) * f_part
                # common_tmp = f_part + F.linear(zf_tmp, self.wv_z) + F.linear(fz_tmp, self.wv_f)
                common_tmp = f_part + torch.diagonal(self.wv_z, 0) * zf_tmp + torch.diagonal(self.wv_f, 0) * fz_tmp
                ...
                # wz_state = new_cell * zf_tmp + common_tmp * wz_state
                # wf_state = new_cell * fz_tmp + common_tmp * wf_state
                wz_state = torch.bmm(zf_tmp.unsqueeze(-1), new_cell.unsqueeze(1)) + common_tmp.unsqueeze(-1) * wz_state
                wf_state = torch.bmm(fz_tmp.unsqueeze(-1), new_cell.unsqueeze(1)) + common_tmp.unsqueeze(-1) * wf_state
    ...
    def forward_learner(self, x, state=None):
        ...
        weight_matrix = torch.cat([self.wm_z, self.wm_f], dim=0)
        # rec_weight_matrix = torch.cat([self.wv_z, self.wv_f], dim=0)
        out = x.view(slen * bsz, x_dim)
        out = F.linear(out, weight_matrix)
        out = out.view(slen, bsz, self.hidden_dim * 2)
        ...
        for z_, f_ in zip(out_z, out_f):
            z_part = torch.tanh(
                z_ + F.linear(new_cell, self.wv_z) + self.bias_z)
            f_part = torch.sigmoid(
                f_ + F.linear(new_cell, self.wv_f) + self.bias_f)
            new_cell = new_cell * f_part + (1. - f_part) * z_part
            output_list.append(new_cell.clone())
    ...
    def compute_grad_rtrl(self, x, rtrl_states, top_gradients, rnn_state_tm1, z_out, f_out):
        ...
        # clearing grad is done in polybeast_learner via optimizer.zero()
        # self.wm_z.grad = None
        # self.wm_f.grad = None
        ...
        # self.bias_z.grad = None
        # self.bias_f.grad = None
        ...
        for x_, top_grad_, c_tm1_, z_, f_ in zip(x, top_gradients, rnn_state_tm1, z_out, f_out):
            ...
            zf_tmp = (1. - f_) * (1. - z_ ** 2)
            fz_tmp = (c_tm1_ - z_) * (1. - f_) * f_
            # common_tmp = f_ + F.linear(zf_tmp, self.wv_z) + F.linear(fz_tmp, self.wv_f)
            common_tmp = f_ + torch.diagonal(self.wv_z, 0) * zf_tmp + torch.diagonal(self.wv_f, 0) * fz_tmp
            ...
            # wz_state = c_tm1_ * zf_tmp + common_tmp * wz_state
            # wf_state = c_tm1_ * fz_tmp + common_tmp * wf_state
            wz_state = torch.bmm(zf_tmp.unsqueeze(-1), c_tm1_.unsqueeze(1)) + common_tmp.unsqueeze(-1) * wz_state
            wf_state = torch.bmm(fz_tmp.unsqueeze(-1), c_tm1_.unsqueeze(1)) + common_tmp.unsqueeze(-1) * wf_state
            ...
            # self.wv_z.grad += (top_grad_ * wz_state).sum(dim=0)
            # self.wv_f.grad += (top_grad_ * wf_state).sum(dim=0)
            self.wv_z.grad += (top_grad_.unsqueeze(-1) * wz_state).sum(dim=0)
            self.wv_f.grad += (top_grad_.unsqueeze(-1) * wf_state).sum(dim=0)
