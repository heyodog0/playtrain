...
let grid = [];
let playerStart, player, goal, obstacles, targets, enemies, bullets, rewards;
...
function updateGame() {
  ...
  for(let i=bullets.length-1; i>=0; i--){
    ...
    if (b.life <= 0) {
      ...
      let wCol = getBulletWallCollision(b.x, b.y, b.r);
      if (wCol.hit) {
        hit = true;
        for (let w of wCol.walls) {
          if (w.x > 0 && w.x < GRID_W - 1 && w.y > 0 && w.y < GRID_H - 1) {
            grid[w.x][w.y] = 0;
            if (rng() < 0.25) {
              rewards.push({x: w.x*CELL + CELL/2, y: w.y*CELL + CELL/2, r: 6});
            }
          }
        }
      }
  ...
  for(let i=rewards.length-1; i>=0; i--){
    let rw = rewards[i];
    if(distSq(player.x, player.y, rw.x, rw.y) < (player.r + rw.r)**2){
      score += 5;
      rewards.splice(i, 1);
    }
  }
...
function getBulletWallCollision(cx, cy, r) {
  let minX = Math.floor((cx - r) / CELL);
  let maxX = Math.floor((cx + r) / CELL);
  let minY = Math.floor((cy - r) / CELL);
  let maxY = Math.floor((cy + r) / CELL);
  
  let res = {hit: false, walls: []};
  for(let x=minX; x<=maxX; x++){
    for(let y=minY; y<=maxY; y++){
      if(x<0 || x>=GRID_W || y<0 || y>=GRID_H || grid[x][y]===1){
        let closestX = clamp(cx, x*CELL, x*CELL+CELL);
