...
function drawEntities() {
  let paddleCenterX = paddle.x + paddle.w / 2 - width / 2;
  let paddleCenterY = paddle.y + paddle.h / 2 - height / 2;
  
  let camX = paddleCenterX * 0.5;
  let camY = paddleCenterY + 120;
  let camZ = 160;
  ...
  if (ball) {
    pointLight(150, 150, 255, ball.x - width/2, ball.y - height/2, 50);
  }
  ...
  push();
  translate(width + 10, height / 2, 10);
  box(20, height + 40, 40);
  pop();
  ...
  for (let i = 0; i < bricks.length; i++) {
    ...
    if (b.active) {
      push();
      translate(b.x + b.w / 2, b.y + b.h / 2, 0);
  ...
  push();
  translate(paddle.x + paddle.w / 2, paddle.y + paddle.h / 2, 0);
  fill(0, 150, 255);
  box(paddle.w, paddle.h, 15);
  
  fill(0, 255, 0);
  for (let i = 0; i < lives; i++) {
    push();
    translate((i - (lives - 1) / 2) * 15, 0, 10);
    box(8, 6, 6);
    pop();
  }
  pop();
  ...
  if (ball) {
    push();
    translate(ball.x, ball.y, ball.r / 2);
    fill(255);
    sphere(ball.r);
    pop();
  }
