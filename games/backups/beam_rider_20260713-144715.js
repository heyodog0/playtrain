...
function draw() {
  ...
  stroke('#2288cc');
  for (let i = 0; i < 20; i++) {
    let z = (i * 2.5) - (gridOffset % 2.5);
    if (z >= 0.1 && z <= 50) {
      let y = getScreenY(z);
      if (y >= VP_Y) { // Start drawing from horizon line
        let sw = Math.max(1, 5 / z); 
        strokeWeight(sw);
        line(0, y, 400, y);
      }
    }
  ...
  // Vertical lane markers / web nodes (Sparce dots matching real game)
  fill('#FFFFFF');
  noStroke();
