...
function draw() {
  ...
  gridOffset = (gridOffset + baseGridSpeed) % 2;
  ...
  if (sentinel) {
    ...
    if (sentinel.x < 20 || sentinel.x > 380) {
      sentinel.dir *= -1; // bounce at edges until killed
    }
  }
  ...
  for (let e of enemies) {
    ...
    if (e.type === 'saucer') {
      fill('#ffffff');
      quad(sx, sy - sz/2, sx + sz, sy, sx, sy + sz/2, sx - sz, sy);
  ...
  fill('#00ffff');
  noStroke();
  triangle(px, py - 20, px + 20, py + 10, px - 20, py + 10);
  fill('#00aaaa');
  triangle(px, py - 5, px + 10, py + 10, px - 10, py + 10);
