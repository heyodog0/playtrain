...
const FRUITS = [
  { r: 14, color: [255, 50, 50] },    // L0
  { r: 20, color: [255, 100, 255] },  // L1
  { r: 28, color: [180, 100, 255] },  // L2
  { r: 36, color: [255, 180, 50] },   // L3
  { r: 46, color: [255, 255, 50] },   // L4
  { r: 58, color: [100, 255, 50] },   // L5
  { r: 72, color: [50, 255, 150] },   // L6
  { r: 88, color: [50, 200, 255] },   // L7
  { r: 106, color: [50, 100, 255] },  // L8
  { r: 126, color: [150, 50, 255] },  // L9
  { r: 140, color: [255, 255, 255] }  // L10
];
...
function draw() {
  ...
  for (let body of bodies) {
    if (!body.isStatic && body.fruitLevel !== undefined) {
      // If fruit is above line and not just spawned
  ...
  // Using template mapping: LEFT=37, RIGHT=39, SPACE=32
  if (keyIsDown(37)) {
    dropperX -= 4;
  }
  if (keyIsDown(39)) {
...
function resetGame(seed) {
  ...
  world = engine.world;
  engine.gravity.y = 1.2; // Slightly higher gravity for difficulty

  // Custom random for Matter.js
  ...
  currentFruitLevel = Math.floor(rng() * 3);
  nextFruitLevel = Math.floor(rng() * 3);
