...
function draw() {
  ...
  // --- Update Grid ---
  let baseGridSpeed = 0.03 + (sector * 0.005);
  ...
  if (!sentinel) {
    ...
    if (spawnTimer <= 0 && enemies.length < 3 + sector && saucersDestroyed < 15) {
      ...
      enemies.push({
        ...
        z: 20,
        prevZ: 20,
        speed: 0.035 + (sector * 0.005) + rng() * 0.015,
        active: true
      });
      spawnTimer = Math.max(40, 100 - sector * 5);
    }
  ...
  // Vertical lane markers (subtle dots instead of heavy lines)
  fill('#3399ee');
  noStroke();
  ...
  for (let e of enemies) {
    ...
    if (e.type === 'saucer') {
      ...
      fill('#cc6600');
      rect(sx - sz, sy - sz, sz*2, sz*2);
    } else if (e.type === 'rejuv') {
      fill('#00ff00');
      rect(sx - sz, sy - sz/2, sz*2, sz);
  ...
  for (let b of bullets) {
    ...
    if (b.type === 'laser') {
      fill('#ffff00');
      let w = Math.max(2, 6 / b.z);
      let h = Math.max(4, 18 / b.z);
      rect(sx - w/2, sy - h, w, h*2);
    } else {
      fill('#00ffff');
