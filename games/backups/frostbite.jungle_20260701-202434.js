...
function resetGame(seed) {
  ...
  // Corner top left and top right trees & bushes
  for (let i = 0; i < 8; i++) {
    let isLeft = rng() > 0.5;
    let xpos = isLeft ? rng() * 100 : 300 + rng() * 100;
    decorations.push({
  ...
  // Corner bottom left and bottom right trees & bushes
  for (let i = 0; i < 12; i++) {
    let isLeft = rng() > 0.5;
    let xpos = isLeft ? rng() * 100 : 300 + rng() * 100;
    decorations.push({
...
function render() {
  ...
  for (let d of decorations) {
    if (playerBaseY === -1 || d.y < playerBaseY) {
      drawDecoration(d);
    }
  }
  ...
  for (let dec of decorations) {
    if (playerBaseY !== -1 && dec.y >= playerBaseY) {
      drawDecoration(dec);
    }
  }
