...
const VP_Y = 100;
const PLAYER_Y = 350;
const BOTTOM_X = [60, 130, 200, 270, 340];
...
function draw() {
  ...
  // --- Render Grid ---
  // Horizontal bounding lines (Perspective grid floor)
  stroke('#2288cc');
  for (let i = 0; i < 15; i++) {
    let z = (i * 3) - (gridOffset % 3);
    if (z >= 0.5 && z <= 40) {
      let y = getScreenY(z);
      let hw = 320 / z; // Wide enough to bound the outer lanes cleanly
      let sw = Math.max(1, 8 / z); 
      strokeWeight(sw);
      line(VP_X - hw, y, VP_X + hw, y);
    }
  }
  ...
  for (let lane = 0; lane < 5; lane++) {
    for (let i = 0; i < 40; i++) {
      let z = (i * 0.8) - (gridOffset % 0.8);
      if (z >= 0.5 && z <= 30) {
        let sy = getScreenY(z);
        let sx = getScreenX(lane, z);
        let sz = Math.max(1.5, 5 / z);
  ...
  for (let e of enemies) {
    ...
    let sz = Math.max(6, 30 / e.z);
    
    noStroke();
    if (e.type === 'saucer') {
      fill('#ffffff');
      quad(sx, sy - sz/3, sx + sz, sy, sx, sy + sz/3, sx - sz, sy);
      fill('#000000');
      rect(sx - sz/4, sy - sz/4, sz/2, sz/2);
  ...
  for (let b of bullets) {
    ...
    let sz = Math.max(2, 6 / b.z);
    
    noStroke();
    if (b.type === 'laser') {
      fill('#ffffff');
      rect(sx - sz/2, sy - sz*1.5, sz, sz*3);
    } else {
      fill('#ff00aa');
      ellipse(sx, sy, sz*2, sz*2.5);
    }
  }
  ...
  // --- Render HUD ---
  // Background left black naturally

  fill('#00ff00');
  for (let i = 0; i < lives; i++) {
    rect(20 + i * 15, 15, 8, 12);
  }
