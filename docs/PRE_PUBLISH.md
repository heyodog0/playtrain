...
Open work items to address **before** publishing `PlayTrain` to npm and PyPI. None of these block local / repo-clone usage today; they only matter once researchers install via `pnpm add PlayTrain` + `pip install PlayTrain` from arbitrary projects.

## 1. Runtime resolution for installed packages

**Problem.** `python/playtrain/env.py` currently resolves the JS runtime via:

```python
DEFAULT_RUNTIME_DIR = Path(__file__).resolve().parents[2] / "runtime"
```

That finds `<repo>/runtime/` when developing against a clone, but a `pip install PlayTrain` lands `env.py` inside `site-packages/`, where no `runtime/` exists. The JS half lives separately under `node_modules/PlayTrain/runtime/`.

**Fix — try in this order:**

1. `runtime_dir=` constructor arg (already supported).
2. `PLAYTRAIN_RUNTIME` env var (already supported).
3. `<repo>/runtime/` for dev clones (already supported).
4. Walk up from `cwd` looking for `node_modules/PlayTrain/runtime/`.
5. Bulletproof fallback: shell out to `node -e "console.log(require.resolve('PlayTrain/package.json'))"` and resolve `runtime/` next to it. This handles pnpm hoisting, monorepos, Yarn PnP, and arbitrary `node_modules` layouts. Cost: one ~50ms `node` subprocess at env startup, once.
...
- **Handshake on startup.** Add a protocol version field to the `ping` response and have `PlayTrainEnv.__init__` compare it against an expected value. Mismatch → clear error message naming both versions.
...
if response.get("protocol") != EXPECTED_PROTOCOL:
    raise RuntimeError(
        f"PlayTrain version mismatch: Python wrapper expects protocol "
...
- **(c) Third package.** Split `PlayTrain-games` as its own npm + PyPI package. Justifiable if/when games grow significantly or third-party game contributions start landing.
...
pnpm add PlayTrain
uv venv && uv pip install PlayTrain
uv run python -c "from playtrain import PlayTrainEnv; e = PlayTrainEnv(game='flappy_bird'); e.reset(seed=0); print('ok')"
```
...
This repo was extracted from `heyodog0/browserless-game-rl` (a.k.a. `fast-llm-games`) on 2026-04-25. The runtime files (`runtime/game-env.mjs`, `runtime/game-worker.mjs`, `runtime/p5/*`, `python/playtrain/env.py`) are the canonical copy here now. If you keep iterating in the parent repo on training/eval but also fix runtime bugs there, those fixes need to be ported here. Decide which repo is the source of truth for runtime code — probably this one — and either:

- Have the parent repo `pip install -e ./PlayTrain` (sibling clone) and stop touching the runtime files there, or
