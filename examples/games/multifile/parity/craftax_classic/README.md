...
This file says what the port claims and how to check it.

## Status

The parity claim holds. The lockstep gate (G2) compares full canonical state,
per-step reward (as float32 bits) and the done flag against PufferLib's C on all
210 corpus episodes, 49,061 steps, and passes. Line coverage of the reference
(G3) is 96.43%: the corpus never reaches iron or diamond.
...
| G5 | `tools/validate.py --game craftax_classic` | — (blocked: the step wire packs `score` as int32, and this is the first game with a fractional score) |
