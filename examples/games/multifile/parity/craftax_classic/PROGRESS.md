...
| 11 | Website: third source dir, overlay, label | human | handoff | build half gated: `6 passed` (`tests/test_website.py`); parity suite `95 passed` | COMMIT11 | **Built; the page needs eyes.** Instructions in README.md under "Website Play tab (handoff)". All three PLAN 8 items done: `playtrain-website/build.sh` unions every `multifile/*/*/dist/` **and copies the `.json` beside the `.js`** (without the sidecar a turn-based game renders at 60fps and is unplayable); `tools/build-pages.mjs` loads it; `tools/play-templates.mjs` uses it for pacing (8fps), the controls overlay, and the parity label from `manifest.reference` including the `not_matched` list. **Catalog untouched** — all 38 pages still 60fps with no label or overlay, asserted by the gate. A malformed sidecar warns and falls back rather than aborting the Play tab. **`playtrain-website` is a separate repo (on `main`) — its `build.sh` change is committed there, not in this branch.** |
