...
| 10a | Study harness reads sidecar actions and pacing | mac | done | `86 passed in 74s` (`tests/test_study_harness.py`); repo suite `95 passed, 3 skipped` | ca91c36 | **G6's blocker is cleared.** `study-templates.mjs` `blockPage` takes `actions` + `stepsPerSecond`; `build-study.mjs` gains `sidecarFor(name)` reading `<name>.json` beside the bundle. `FRAME_MS` is now `1000 / STEPS_PER_SECOND` (8 for this game). The default8 quantizer is **untouched** and still the code path for every catalog game — a second, generic quantizer is derived from the action table and used only when the table is not default8. It mirrors 90_playtrain.js: press beats held, recency within each class. A gate proves all 17 actions are reachable from their own key, and that the 9 existing study blocks still render `IS_DEFAULT8 = true` at 60 steps/s, since re-timing them would invalidate collected data. Sidecar `human.controls` is the fallback when study-config has no copy for the game. |
| 10b | Human quickplay session, replay-verified | human | handoff | prerequisites gated: `9 passed` (`tests/test_study_harness.py`) | d2537cf | **Ready for a person. Do not simulate.** Instructions are in README.md under "G6: the human session (handoff)". `node study/quickplay.mjs craftax_classic --seconds 100` — no `--games` needed, quickplay now searches the multifile dists. Verified for them: the game resolves by bare name, the block page renders at 8 steps/s with the 17-action table, and the **headless replay path picks the sidecar table up automatically and is deterministic** (226 steps, score 0.9999998, identical across runs) — `verify-replay.mjs` builds a bare `GameEnv({gamePath, frameSkip, maxSteps})` with no action table, so that only works because of the 7b sidecar loading. G6 is **not** affected by the 8b int32-score blocker: it compares `getGameState().score` in-process, not across the Python step wire. |
| 11 | Website: third source dir, overlay, label | human | handoff | build half gated: `6 passed` (`tests/test_website.py`); parity suite `95 passed` | 4f6c2a8 | **Built; the page needs eyes.** Instructions in README.md under "Website Play tab (handoff)". All three PLAN 8 items done: `playtrain-website/build.sh` unions every `multifile/*/*/dist/` **and copies the `.json` beside the `.js`** (without the sidecar a turn-based game renders at 60fps and is unplayable); `tools/build-pages.mjs` loads it; `tools/play-templates.mjs` uses it for pacing (8fps), the controls overlay, and the parity label from `manifest.reference` including the `not_matched` list. **Catalog untouched** — all 38 pages still 60fps with no label or overlay, asserted by the gate. A malformed sidecar warns and falls back rather than aborting the Play tab. **`playtrain-website` is a separate repo (on `main`) — its `build.sh` change is committed there, not in this branch.** |
| 12 | Throughput bench on paper node; first PPO run | cluster | blocked | not run | | **Same wall as 9b**, see "9b blocker" below: this branch is unpushed (34+ commits) and the only FASRC checkout is on `main` at `29e1e7f`, dirty, predating the engine-tier work. Nothing to bench there until a clean checkout exists. Pushing `release` is the user's call, so it is not done here. One number is already measured on the Mac and is a useful sanity reference: **12815 FPS single-env** (validate.py check 5, 0.08 ms/step) — see the 8b write-up. |
| 13a | `85_obs_symbolic.js` + G2 extended to the 1345-vector | mac | done | `99 passed in 85.08s` (`tests/test_obs_symbolic.py`) | 7f8fa0d | **All 1345 floats bit-identical to the C, every step of all 210 episodes.** New driver mode `cc_ref obs` (full steps, then an explicit `compute_observations` — `cc_step_no_reset` stops before `puf_step`'s own call). Added to the manifest and the bundle. Mutation-checked 4 ways, 3 caught; the 4th (dropping `F()` on the /10 scalings) is **equivalent over the values the game produces** — `F(n*F(0.1))` and `F(n*0.1)` differ for 48 of 256 int8 values but only at `\|n\| >= 67`, and inventory is clamped 0..9 with min health anywhere in the corpus at -3. Noted in the source so the next person does not re-derive it. The coverage test samples **every 5th episode, not the first 40** — the first 100 are all `uniform`, which made 8 tail floats look permanently dead; spread across policies the union is 785/1345 and only 5 stay constant, exactly the 3b-ii iron/diamond gap. |
| 13b | `obs_mode="symbolic"` in `PlayTrainEnv` + the node host | mac | done | `107 passed in 85.03s` (`tests/test_obs_mode.py`) | f1e2ed6 | `PlayTrainEnv(game="craftax_classic", obs_mode="symbolic")` gives `Box(-inf, inf, (1345,), float32)`, and the vector reaching the env is **bit-identical to `cc_ref obs`** over 120 steps. The rasterizer is skipped entirely. Dimension comes from the sidecar's `obs.symbolic`; a game without it is refused at construction rather than handed a wrong-shaped buffer. Frame stacking is not applied (a symbolic vector is not a frame). Pixel mode unchanged. `85_obs_symbolic.js` exports `computeSymbolicObs(st)`; `90_playtrain.js` wraps it as the no-arg `getObservation()` the host contract wants. |
| 13c | `obs_mode="symbolic"` in the two C++ hosts | mac | done | `115 passed in 86.50s` (`tests/test_obs_mode.py`, `tests/test_engines.py`); repo suite `95 passed, 3 skipped` | COMMIT13C | **All three hosts now do symbolic, and agree.** `qjs_vec_host`: `NativeVecEnv(obs_mode="symbolic")` gives `(N, 1345) float32`, **bit-identical to `cc_ref obs`** over 80 steps. `qjs_host`: opt in with `PLAYTRAIN_QJS_OBS_MODE=symbolic` (+ `PLAYTRAIN_OBS_MODE` for `reference_trace.mjs`), which makes `gate_qjs.sh` hash the 1345-float vector instead of the frame — **V8 vs QuickJS bit-exact, 600 steps x 3 seeds**. ABI is **additive**: a new `vec_set_obs_symbolic(h, dim)` rather than a change to `vec_create`, so the trainers' callers are untouched; the Python side degrades with a clear error on an older `.so`. Pixel mode is the default everywhere and unchanged. Both native libs were rebuilt. |
...
- 2026-09-14 — 10a — study harness now sidecar-driven. 10b (the human session) is a handoff: prerequisites are in place, a person has to play it.
- 2026-09-14 — 10b — handoff prepared, not performed. quickplay.mjs now resolves multifile dists; its stale "same Discrete(8) quantizer" comment corrected after the 10a change.
- 2026-09-14 — 11 — website build half done in two repos; visual check left to a human, as the task says.
