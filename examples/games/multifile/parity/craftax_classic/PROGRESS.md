...
| 13b | `obs_mode="symbolic"` in `PlayTrainEnv` + the node host | mac | done | `107 passed in 85.03s` (`tests/test_obs_mode.py`) | COMMIT13B | `PlayTrainEnv(game="craftax_classic", obs_mode="symbolic")` gives `Box(-inf, inf, (1345,), float32)`, and the vector reaching the env is **bit-identical to `cc_ref obs`** over 120 steps. The rasterizer is skipped entirely. Dimension comes from the sidecar's `obs.symbolic`; a game without it is refused at construction rather than handed a wrong-shaped buffer. Frame stacking is not applied (a symbolic vector is not a frame). Pixel mode unchanged. `85_obs_symbolic.js` exports `computeSymbolicObs(st)`; `90_playtrain.js` wraps it as the no-arg `getObservation()` the host contract wants. |
| 13c | `obs_mode="symbolic"` in the two C++ hosts (`qjs_host`, `qjs_vec_host`) | mac | todo | | | PLAN 3.6 says "the three hosts"; 13b did the node one. The C++ pair need the same: call the game's `getObservation()`, copy the Float32Array buffer into the obs slot, skip the rasterizer. Not started — doing it badly would be worse than not doing it, and the node path already makes symbolic training reachable. |
