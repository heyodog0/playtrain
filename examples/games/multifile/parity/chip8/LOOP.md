...
You are executing `examples/games/multifile/parity/chip8/PLAN.md` in the
...
2. Read PLAN.md sections 2, 3, 4 and 7 in full before writing code, and the
   sibling family `examples/games/multifile/parity/vgdl/` for the conventions
   (manifest, bundler, sidecars, goldens, gate scripts, tests). Reuse its
   shapes; do not invent new ones. If PLAN.md and the Octax source disagree,
   the Octax source wins and you correct PLAN.md in the same commit with a note.
3. Do the unit. Run its gate, the one named in the unit's "done when" column.
   Python in the playtrain repo is always `uv run --no-sync`; the oracle runs
   in its own `uv venv`. A unit is done only when its gate is green on this
   machine.
...
5. Update `PROGRESS.md`: the ledger row (status, gate output last line, commit
   hash, notes), any new reference quirk, any G9 number, one iteration-log
   line, `ITERATION`, `LAST_COMMIT`. Commit that too. Then end the iteration.

Rules that override everything else:

- **Never weaken a gate to pass it.** Full state every step means every
  register, the stack, both timers, the keypad, the display hash, score and
  terminated. No tolerances, no dropped fields, no shortened corpus, no
  `xfail`, no `skip` outside the documented "oracle not configured" and
  "playwright absent" skips. If a gate cannot pass, mark the unit `blocked`
  with the exact first divergence (game, seed, step, field, Octax value, JS
  value) and end the iteration.
- **Never edit the Octax checkout or the ROMs.** They are the reference. If
  Octax has a bug, reproduce it, record it under "Reference quirks", cite the
  file and line, and do not fix it.
- **Never change the runtime, hosts, rasterizer or catalog.** The VGDL branch
  already added everything this port needs (`drawTiles`, rebuilt hosts,
  sidecar handling). If something is genuinely missing, that is a `blocked`
  note naming the file and the change, not a change.
- **Integer-only JS.** Uint8Array/Uint16Array state, `>>> 0` after every
  32-bit op, no floats in the CPU, no `BigInt`. V8 and QuickJS must agree by
  construction; `native/gate_qjs.sh` checks that they do.
...
- **Do not build a compiler, a JIT, or a native twin.** PLAN.md section 6:
  measure first; the interpreter is expected to be enough.
