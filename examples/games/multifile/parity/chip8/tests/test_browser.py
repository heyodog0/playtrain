"""G8: the bundles play in a real browser: the built page loads, steps, takes the game's keypad key
and draws the two display colours, with the sidecar's controls overlay. Skips unless playwright-core
and a Chromium are available: playwright-core resolvable from the family dir or from
PLAYWRIGHT_CORE_DIR (a scratch `npm i playwright-core`), and PLAYWRIGHT_CHROMIUM pointing at a
...
GAMES = ["chip8_brix", "chip8_tetris", "chip8_blinky"]
...
def test_pages_play_in_headless_chromium():
    out = Path(tempfile.mkdtemp(prefix="chip8_pages_"))
