...
const outDir = check ? join(tmpdir(), 'chip8_bundle_check') : join(ROOT, 'dist');
...
for (const game of games) {
  execFileSync('node', [join(HERE, 'bundle_chip8.mjs'), game, '--sidecar', '--out', join(outDir, `chip8_${game}.js`)], { stdio: 'pipe' });
  built.push(`chip8_${game}`);
}
if (check) {
  let stale = 0;
