// vgdl/infer/jaws — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 8 types, 14 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_whalehole = T[2];   // whalehole > SpawnPoint
  const t_piranhahole = T[3];   // piranhahole > SpawnPoint
  const t_avatar = T[4];   // avatar > MovingAvatar
  const t_shark = T[5];   // shark > Chaser
  const t_whale = T[6];   // whale > Missile
  const t_piranha = T[7];   // piranha > Missile
  ...
  function tick(keys) {
    ...
    if (t_whalehole.jpDirty) { for (let k = 0; k < t_whalehole.n; k++) t_whalehole.jpT[t_whalehole.live[k]] = -1; t_whalehole.jpDirty = false; }
    if (t_piranhahole.jpDirty) { for (let k = 0; k < t_piranhahole.n; k++) t_piranhahole.jpT[t_piranhahole.live[k]] = -1; t_piranhahole.jpDirty = false; }
    if (t_avatar.jpDirty) { for (let k = 0; k < t_avatar.n; k++) t_avatar.jpT[t_avatar.live[k]] = -1; t_avatar.jpDirty = false; }
    if (t_shark.jpDirty) { for (let k = 0; k < t_shark.n; k++) t_shark.jpT[t_shark.live[k]] = -1; t_shark.jpDirty = false; }
    if (t_whale.jpDirty) { for (let k = 0; k < t_whale.n; k++) t_whale.jpT[t_whale.live[k]] = -1; t_whale.jpDirty = false; }
    if (t_piranha.jpDirty) { for (let k = 0; k < t_piranha.n; k++) t_piranha.jpT[t_piranha.live[k]] = -1; t_piranha.jpDirty = false; }
    const n0_whalehole = t_whalehole.n;
    const n0_piranhahole = t_piranhahole.n;
    const n0_avatar = t_avatar.n;
    const n0_shark = t_shark.n;
    const n0_whale = t_whale.n;
    const n0_piranha = t_piranha.n;
    for (let k = 0; k < n0_whalehole; k++) vgUpdSpawn(t_whalehole, t_whalehole.live[k]);   // whalehole > SpawnPoint
    for (let k = 0; k < n0_piranhahole; k++) vgUpdSpawn(t_piranhahole, t_piranhahole.live[k]);   // piranhahole > SpawnPoint
    for (let k = 0; k < n0_avatar; k++) vgUpdMoving(t_avatar, t_avatar.live[k]);   // avatar > MovingAvatar
    for (let k = 0; k < n0_shark; k++) vgUpdChaser(t_shark, t_shark.live[k]);   // shark > Chaser
    for (let k = 0; k < n0_whale; k++) vgBaseUpdate(t_whale, t_whale.live[k]);   // whale > Missile
    for (let k = 0; k < n0_piranha; k++) vgBaseUpdate(t_piranha, t_piranha.live[k]);   // piranha > Missile
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // shark wall > stepBack
    if (t_shark.n !== 0 && t_wall.n !== 0) {
      if (t_shark.n <= t_wall.n) { for (let k = 0; k < t_shark.n; k++) { const i = t_shark.live[k]; vgCollectType(t_shark, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_shark.killed[i]) { VG_EFFECTS.stepBack(t_shark, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_shark); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_shark.killed[j]) { VG_EFFECTS.stepBack(t_shark, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // avatar shark > killSprite
    if (t_avatar.n !== 0 && t_shark.n !== 0) {
      if (t_avatar.n <= t_shark.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_shark); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_shark.n; k++) { const i = t_shark.live[k]; vgCollectType(t_shark, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar whale > killSprite
    if (t_avatar.n !== 0 && t_whale.n !== 0) {
      if (t_avatar.n <= t_whale.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_whale); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_whale.n; k++) { const i = t_whale.live[k]; vgCollectType(t_whale, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar piranha > killSprite
    if (t_avatar.n !== 0 && t_piranha.n !== 0) {
      if (t_avatar.n <= t_piranha.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_piranha); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_piranha.n; k++) { const i = t_piranha.live[k]; vgCollectType(t_piranha, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // whale wall > killSprite
    if (t_whale.n !== 0 && t_wall.n !== 0) {
      if (t_whale.n <= t_wall.n) { for (let k = 0; k < t_whale.n; k++) { const i = t_whale.live[k]; vgCollectType(t_whale, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_whale.killed[i]) { vgKill(t_whale, i); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_whale); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_whale.killed[j]) { vgKill(t_whale, j); } } } }
    }
    // piranha wall > killSprite
    if (t_piranha.n !== 0 && t_wall.n !== 0) {
      if (t_piranha.n <= t_wall.n) { for (let k = 0; k < t_piranha.n; k++) { const i = t_piranha.live[k]; vgCollectType(t_piranha, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_piranha.killed[i]) { vgKill(t_piranha, i); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_piranha); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_piranha.killed[j]) { vgKill(t_piranha, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // piranha EOS > killSprite
    for (let k = t_piranha.n - 1; k >= 0; k--) { const i = t_piranha.live[k]; if (vgContains(t_piranha, i)) continue; vgKill(t_piranha, i); if (!vgContains(t_piranha, i)) vgKill(t_piranha, i); }
    // piranhahole EOS > killSprite
    for (let k = t_piranhahole.n - 1; k >= 0; k--) { const i = t_piranhahole.live[k]; if (vgContains(t_piranhahole, i)) continue; vgKill(t_piranhahole, i); if (!vgContains(t_piranhahole, i)) vgKill(t_piranhahole, i); }
    // shark EOS > killSprite
    for (let k = t_shark.n - 1; k >= 0; k--) { const i = t_shark.live[k]; if (vgContains(t_shark, i)) continue; vgKill(t_shark, i); if (!vgContains(t_shark, i)) vgKill(t_shark, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    // whale EOS > killSprite
    for (let k = t_whale.n - 1; k >= 0; k--) { const i = t_whale.live[k]; if (vgContains(t_whale, i)) continue; vgKill(t_whale, i); if (!vgContains(t_whale, i)) vgKill(t_whale, i); }
    // whalehole EOS > killSprite
    for (let k = t_whalehole.n - 1; k >= 0; k--) { const i = t_whalehole.live[k]; if (vgContains(t_whalehole, i)) continue; vgKill(t_whalehole, i); if (!vgContains(t_whalehole, i)) vgKill(t_whalehole, i); }
    VG.ss = null;
    vgFlush();
