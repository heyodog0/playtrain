...
const VG_GROUP_ORDER = ["wall", "box", "hole", "avatar"];
...
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  function events() {
    ...
    while (again) {
      const nc = new Set();
      let g_avatar = null, g_hole = null, g_box = null, g_wall = null;   // group snapshots, one lookup per stype per pass
      // avatar hole > nothing
      ...
      { const e1 = g_box || (g_box = rcGroupEntry("box", cache)), l1 = e1.list;
        ...
        g_avatar = null; g_hole = null; g_box = null; g_wall = null;   // _updateCollisionDict may have invalidated snapshots
      }
      // box wall > stepBack
      { const e1 = g_box || (g_box = rcGroupEntry("box", cache)), l1 = e1.list;
        const e2 = g_wall || (g_wall = rcGroupEntry("wall", cache));
  ...
  function tick(keys) {
    ...
    VG.ended = false; VG.won = false;
    if (rcLiveCount(t_box) <= 0) { VG.ended = true; VG.won = true; return; }   // SpriteCounter stype=box limit=0 win=true
  }
  VG.tickImpl = tick;
};
