// vgdl/infer/missile_command — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
const VG_LEVELS = ["wwwwwwwwwwwwwwwwwwwwwwwww\nw...m...m...m...m...m...w\nw.......................w\nw.......................w\nw.......................w\nw........m..............w\nw.......................w\nw...........A...........w\nw.......................w\nw..............m........w\nw.......................w\nw...c....c......c...c...w\nwwwwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwwwww\nw....f..f......f..f....w\nw......................w\nw...........f..........w\nw......................w\nw......................w\nw......................w\nw...........A..........w\nw......................w\nw......................w\nw......................w\nw...c.c....c....c.c....w\nwwwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwwwww\nw....m.....m........f..w\nw......................w\nw......................w\nw......f...............w\nw......................w\nw......................w\nw...........A..........w\nw......................w\nw......................w\nw......................w\nw...c.......c......c...w\nwwwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwwwww\nw.f........m....m.....fw\nw......................w\nwwwwww..m..............w\nw......................w\nw...................wwww\nw......................w\nw...........A..........w\nw..www.........www.....w\nw..w............w......w\nw..w............w......w\nw..w..c.....c...w..c...w\nwwwwwwwwwwwwwwwwwwwwwwww\n"];
const VG_BLOCK = 50;
...
// ---- compiled tick: 7 types, 16 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_city = T[1];   // city > RandomNPC
  const t_explosion = T[2];   // explosion > Flicker
  const t_avatar = T[3];   // avatar > ShootAvatar
  const t_incoming_slow = T[4];   // incoming_slow > Chaser
  const t_incoming_fast = T[5];   // incoming_fast > Chaser
  ...
  function tick(keys) {
    ...
    if (t_city.jpDirty) { for (let k = 0; k < t_city.n; k++) t_city.jpT[t_city.live[k]] = -1; t_city.jpDirty = false; }
    if (t_explosion.jpDirty) { for (let k = 0; k < t_explosion.n; k++) t_explosion.jpT[t_explosion.live[k]] = -1; t_explosion.jpDirty = false; }
    if (t_avatar.jpDirty) { for (let k = 0; k < t_avatar.n; k++) t_avatar.jpT[t_avatar.live[k]] = -1; t_avatar.jpDirty = false; }
    if (t_incoming_slow.jpDirty) { for (let k = 0; k < t_incoming_slow.n; k++) t_incoming_slow.jpT[t_incoming_slow.live[k]] = -1; t_incoming_slow.jpDirty = false; }
    if (t_incoming_fast.jpDirty) { for (let k = 0; k < t_incoming_fast.n; k++) t_incoming_fast.jpT[t_incoming_fast.live[k]] = -1; t_incoming_fast.jpDirty = false; }
    const n0_city = t_city.n;
    const n0_explosion = t_explosion.n;
    const n0_avatar = t_avatar.n;
    const n0_incoming_slow = t_incoming_slow.n;
    const n0_incoming_fast = t_incoming_fast.n;
    for (let k = 0; k < n0_city; k++) vgUpdRandom(t_city, t_city.live[k]);   // city > RandomNPC
    for (let k = 0; k < n0_explosion; k++) vgUpdFlicker(t_explosion, t_explosion.live[k]);   // explosion > Flicker
    for (let k = 0; k < n0_avatar; k++) vgUpdShoot(t_avatar, t_avatar.live[k]);   // avatar > ShootAvatar
    for (let k = 0; k < n0_incoming_slow; k++) vgUpdChaser(t_incoming_slow, t_incoming_slow.live[k]);   // incoming_slow > Chaser
    for (let k = 0; k < n0_incoming_fast; k++) vgUpdChaser(t_incoming_fast, t_incoming_fast.live[k]);   // incoming_fast > Chaser
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // incoming_slow wall > stepBack
    if (t_incoming_slow.n !== 0 && t_wall.n !== 0) {
      if (t_incoming_slow.n <= t_wall.n) { for (let k = 0; k < t_incoming_slow.n; k++) { const i = t_incoming_slow.live[k]; vgCollectType(t_incoming_slow, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[i]) { VG_EFFECTS.stepBack(t_incoming_slow, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_incoming_slow); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[j]) { VG_EFFECTS.stepBack(t_incoming_slow, j, t_wall, i, undefined); } } } }
    }
    // incoming_fast wall > stepBack
    if (t_incoming_fast.n !== 0 && t_wall.n !== 0) {
      if (t_incoming_fast.n <= t_wall.n) { for (let k = 0; k < t_incoming_fast.n; k++) { const i = t_incoming_fast.live[k]; vgCollectType(t_incoming_fast, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[i]) { VG_EFFECTS.stepBack(t_incoming_fast, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_incoming_fast); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[j]) { VG_EFFECTS.stepBack(t_incoming_fast, j, t_wall, i, undefined); } } } }
    }
    // avatar wall > stepBack
    ...
    // city wall > stepBack
    if (t_city.n !== 0 && t_wall.n !== 0) {
      if (t_city.n <= t_wall.n) { for (let k = 0; k < t_city.n; k++) { const i = t_city.live[k]; vgCollectType(t_city, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[i]) { VG_EFFECTS.stepBack(t_city, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_city); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[j]) { VG_EFFECTS.stepBack(t_city, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // incoming_slow wall > stepBack
    if (t_incoming_slow.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // incoming_slow city > killSprite scoreChange=-1
    if (t_incoming_slow.n !== 0 && t_city.n !== 0) {
      if (t_incoming_slow.n <= t_city.n) { for (let k = 0; k < t_incoming_slow.n; k++) { const i = t_incoming_slow.live[k]; vgCollectType(t_incoming_slow, i, t_city); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[i]) { VG.score += -1; vgKill(t_incoming_slow, i); } } } }
      else { for (let k = 0; k < t_city.n; k++) { const i = t_city.live[k]; vgCollectType(t_city, i, t_incoming_slow); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[j]) { VG.score += -1; vgKill(t_incoming_slow, j); } } } }
    }
    // city incoming_slow > killSprite
    if (t_city.n !== 0 && t_incoming_slow.n !== 0) {
      if (t_city.n <= t_incoming_slow.n) { for (let k = 0; k < t_city.n; k++) { const i = t_city.live[k]; vgCollectType(t_city, i, t_incoming_slow); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[i]) { vgKill(t_city, i); } } } }
      else { for (let k = 0; k < t_incoming_slow.n; k++) { const i = t_incoming_slow.live[k]; vgCollectType(t_incoming_slow, i, t_city); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[j]) { vgKill(t_city, j); } } } }
    }
    // incoming_fast city > killSprite scoreChange=-1
    if (t_incoming_fast.n !== 0 && t_city.n !== 0) {
      if (t_incoming_fast.n <= t_city.n) { for (let k = 0; k < t_incoming_fast.n; k++) { const i = t_incoming_fast.live[k]; vgCollectType(t_incoming_fast, i, t_city); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[i]) { VG.score += -1; vgKill(t_incoming_fast, i); } } } }
      else { for (let k = 0; k < t_city.n; k++) { const i = t_city.live[k]; vgCollectType(t_city, i, t_incoming_fast); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[j]) { VG.score += -1; vgKill(t_incoming_fast, j); } } } }
    }
    // city incoming_fast > killSprite
    if (t_city.n !== 0 && t_incoming_fast.n !== 0) {
      if (t_city.n <= t_incoming_fast.n) { for (let k = 0; k < t_city.n; k++) { const i = t_city.live[k]; vgCollectType(t_city, i, t_incoming_fast); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[i]) { vgKill(t_city, i); } } } }
      else { for (let k = 0; k < t_incoming_fast.n; k++) { const i = t_incoming_fast.live[k]; vgCollectType(t_incoming_fast, i, t_city); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_city.killed[j]) { vgKill(t_city, j); } } } }
    }
    // incoming_slow explosion > killSprite scoreChange=1
    if (t_incoming_slow.n !== 0 && t_explosion.n !== 0) {
      if (t_incoming_slow.n <= t_explosion.n) { for (let k = 0; k < t_incoming_slow.n; k++) { const i = t_incoming_slow.live[k]; vgCollectType(t_incoming_slow, i, t_explosion); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[i]) { VG.score += 1; vgKill(t_incoming_slow, i); } } } }
      else { for (let k = 0; k < t_explosion.n; k++) { const i = t_explosion.live[k]; vgCollectType(t_explosion, i, t_incoming_slow); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_slow.killed[j]) { VG.score += 1; vgKill(t_incoming_slow, j); } } } }
    }
    // incoming_fast explosion > killSprite scoreChange=1
    if (t_incoming_fast.n !== 0 && t_explosion.n !== 0) {
      if (t_incoming_fast.n <= t_explosion.n) { for (let k = 0; k < t_incoming_fast.n; k++) { const i = t_incoming_fast.live[k]; vgCollectType(t_incoming_fast, i, t_explosion); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[i]) { VG.score += 1; vgKill(t_incoming_fast, i); } } } }
      else { for (let k = 0; k < t_explosion.n; k++) { const i = t_explosion.live[k]; vgCollectType(t_explosion, i, t_incoming_fast); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_incoming_fast.killed[j]) { VG.score += 1; vgKill(t_incoming_fast, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // city EOS > killSprite
    for (let k = t_city.n - 1; k >= 0; k--) { const i = t_city.live[k]; if (vgContains(t_city, i)) continue; vgKill(t_city, i); if (!vgContains(t_city, i)) vgKill(t_city, i); }
    // explosion EOS > killSprite
    for (let k = t_explosion.n - 1; k >= 0; k--) { const i = t_explosion.live[k]; if (vgContains(t_explosion, i)) continue; vgKill(t_explosion, i); if (!vgContains(t_explosion, i)) vgKill(t_explosion, i); }
    // incoming_fast EOS > killSprite
    for (let k = t_incoming_fast.n - 1; k >= 0; k--) { const i = t_incoming_fast.live[k]; if (vgContains(t_incoming_fast, i)) continue; vgKill(t_incoming_fast, i); if (!vgContains(t_incoming_fast, i)) vgKill(t_incoming_fast, i); }
    // incoming_slow EOS > killSprite
    for (let k = t_incoming_slow.n - 1; k >= 0; k--) { const i = t_incoming_slow.live[k]; if (vgContains(t_incoming_slow, i)) continue; vgKill(t_incoming_slow, i); if (!vgContains(t_incoming_slow, i)) vgKill(t_incoming_slow, i); }
    // wall EOS > killSprite
    ...
    VG.ended = false; VG.won = false;
    if (t_city.n <= 0) { VG.ended = true; VG.won = false; }   // SpriteCounter stype=city win=false
    else if ((t_incoming_slow.n + t_incoming_fast.n) === 0) { VG.ended = true; VG.won = true; }   // MultiSpriteCounter stype1=incoming_slow stype2=incoming_fast win=true
    else if (VG.time >= 2000) { VG.ended = true; VG.won = false; }   // Timeout limit=2000 win=false
  }
  VG.tickImpl = tick;
};
