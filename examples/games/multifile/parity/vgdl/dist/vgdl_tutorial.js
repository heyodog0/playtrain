// vgdl/infer/tutorial — GENERATED by tools/bundle_vgdl.mjs, do not edit.
// reference: py-vgdl (see manifest.json); block=50 render=tiles levels=2
...
const VG_LEVELS = ["wwwwwwwwwwwwwwwwwwwwwwwwww\nwA.......................w\nw.......t........t.......w\nw........................w\nw........................w\nw....................a...w\nw..........a.............w\nw........................w\nw.........aaaaa..........w\nwwwwwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwwwwwww\nwA..p....................w\nw................c.......w\nw......t.....a...........w\nwwww.....................w\nw.aw.................a...w\nw.ww.......a.............w\nw..............c.........w\nw....................t...w\nwwwwwwwwwwwwwwwwwwwwwwwwww\n"];
const VG_BLOCK = 50;
...
// ---- compiled tick: 8 types, 5 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_thing = T[1];   // thing > RandomNPC
  const t_thingtwo = T[2];   // thingtwo > Immovable (inert: never updated, never leaves the screen)
  const t_thingthree = T[3];   // thingthree > Immovable (inert: never updated, never leaves the screen)
  const t_apple = T[4];   // apple > Immovable (inert: never updated, never leaves the screen)
  const t_bullet = T[5];   // bullet > Flicker
  const t_avatar = T[6];   // avatar > ShootAvatar
  const t_wall = T[7];   // wall > Immovable (inert: never updated, never leaves the screen)
  const ARGS_0 = {"stype":"thingtwo"};
  ...
  function tick(keys) {
    ...
    if (t_thing.jpDirty) { for (let k = 0; k < t_thing.n; k++) t_thing.jpT[t_thing.live[k]] = -1; t_thing.jpDirty = false; }
    if (t_bullet.jpDirty) { for (let k = 0; k < t_bullet.n; k++) t_bullet.jpT[t_bullet.live[k]] = -1; t_bullet.jpDirty = false; }
    if (t_avatar.jpDirty) { for (let k = 0; k < t_avatar.n; k++) t_avatar.jpT[t_avatar.live[k]] = -1; t_avatar.jpDirty = false; }
    const n0_thing = t_thing.n;
    const n0_bullet = t_bullet.n;
    const n0_avatar = t_avatar.n;
    for (let k = 0; k < n0_thing; k++) vgUpdRandom(t_thing, t_thing.live[k]);   // thing > RandomNPC
    for (let k = 0; k < n0_bullet; k++) vgUpdFlicker(t_bullet, t_bullet.live[k]);   // bullet > Flicker
    ...
    // thing wall > stepBack
    if (t_thing.n !== 0 && t_wall.n !== 0) {
      if (t_thing.n <= t_wall.n) { for (let k = 0; k < t_thing.n; k++) { const i = t_thing.live[k]; vgCollectType(t_thing, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_thing.killed[i]) { VG_EFFECTS.stepBack(t_thing, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_thing); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_thing.killed[j]) { VG_EFFECTS.stepBack(t_thing, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // thing bullet > transformTo stype=thingtwo
    if (t_thing.n !== 0 && t_bullet.n !== 0) {
      if (t_thing.n <= t_bullet.n) { for (let k = 0; k < t_thing.n; k++) { const i = t_thing.live[k]; vgCollectType(t_thing, i, t_bullet); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_thing.killed[i]) { VG_EFFECTS.transformTo(t_thing, i, t_bullet, j, ARGS_0); } } } }
      else { for (let k = 0; k < t_bullet.n; k++) { const i = t_bullet.live[k]; vgCollectType(t_bullet, i, t_thing); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_thing.killed[j]) { VG_EFFECTS.transformTo(t_thing, j, t_bullet, i, ARGS_0); } } } }
    }
    // apple avatar > killSprite
    if (t_apple.n !== 0 && t_avatar.n !== 0) {
      if (t_apple.n <= t_avatar.n) { for (let k = 0; k < t_apple.n; k++) { const i = t_apple.live[k]; vgCollectType(t_apple, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_apple.killed[i]) { vgKill(t_apple, i); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_apple); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_apple.killed[j]) { vgKill(t_apple, j); } } } }
    }
    // avatar thingthree > killSprite
    if (t_avatar.n !== 0 && t_thingthree.n !== 0) {
      if (t_avatar.n <= t_thingthree.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_thingthree); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_thingthree.n; k++) { const i = t_thingthree.live[k]; vgCollectType(t_thingthree, i, t_avatar); const nc = vgCand.length;
    ...
    else if (t_thing.n <= 0) { VG.ended = true; VG.won = false; }   // SpriteCounter stype=thing limit=0 win=false
    else if (t_apple.n <= 0) { VG.ended = true; VG.won = true; }   // SpriteCounter stype=apple limit=0 win=true
