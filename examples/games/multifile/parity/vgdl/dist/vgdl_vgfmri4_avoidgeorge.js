...
const VG_GROUP_ORDER = ["wall", "quiet", "annoyed", "cigarette", "avatar", "george"];
...
// ---- compiled tick: 6 types, 9 rules (tools/compile_vgdl.mjs, profile rcrl) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  const T = VG.types;
  const t_annoyed = T[0];   // annoyed > RandomNPC
  const t_quiet = T[1];   // quiet > RandomNPC
  const t_george = T[2];   // george > Chaser
  const t_cigarette = T[3];   // cigarette > Flicker
  const t_wall = T[4];   // wall > Immovable (inert)
  ...
  function events() {
    ...
    while (again) {
      const nc = new Set();
      let g_annoyed = null, g_cigarette = null, g_avatar = null, g_george = null, g_wall = null, g_quiet = null;   // group snapshots, one lookup per stype per pass
      // annoyed cigarette > changeScore value=1
      { const e1 = g_annoyed || (g_annoyed = rcGroupEntry("annoyed", cache)), l1 = e1.list;
        const e2 = g_cigarette || (g_cigarette = rcGroupEntry("cigarette", cache));
      ...
      { const e1 = g_avatar || (g_avatar = rcGroupEntry("avatar", cache)), l1 = e1.list;
        const e2 = g_george || (g_george = rcGroupEntry("george", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
      ...
      // quiet wall > stepBack
      { const e1 = g_quiet || (g_quiet = rcGroupEntry("quiet", cache)), l1 = e1.list;
        const e2 = g_wall || (g_wall = rcGroupEntry("wall", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
          for (let u = 0; u < l1.length; u += 2) { const s1 = l1[u], i1 = l1[u + 1]; rcCollect(s1, i1, e2); if (rcCand.length === 0) continue; const cand = rcCand.slice();
      ...
      // george wall > stepBack
      { const e1 = g_george || (g_george = rcGroupEntry("george", cache)), l1 = e1.list;
        const e2 = g_wall || (g_wall = rcGroupEntry("wall", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
          for (let u = 0; u < l1.length; u += 2) { const s1 = l1[u], i1 = l1[u + 1]; rcCollect(s1, i1, e2); if (rcCand.length === 0) continue; const cand = rcCand.slice();
  ...
  function tick(keys) {
    ...
    const noop = keys !== null && keys.length === 0;
    VG.keys = keys || [];
    for (let k = 0; k < t_annoyed.n; k++) { const i = t_annoyed.live[k]; if (!t_annoyed.killed[i]) rcUpdRandom(t_annoyed, i); }   // annoyed > RandomNPC
    for (let k = 0; k < t_quiet.n; k++) { const i = t_quiet.live[k]; if (!t_quiet.killed[i]) rcUpdRandom(t_quiet, i); }   // quiet > RandomNPC
    for (let k = 0; k < t_george.n; k++) { const i = t_george.live[k]; if (!t_george.killed[i]) rcUpdChaser(t_george, i); }   // george > Chaser
    for (let k = 0; k < t_cigarette.n; k++) { const i = t_cigarette.live[k]; if (!t_cigarette.killed[i]) rcUpdFlicker(t_cigarette, i); }   // cigarette > Flicker
    ...
    if (rcLiveCount(t_quiet) <= 0) { VG.ended = true; VG.won = false; return; }   // SpriteCounter stype=quiet win=false
    if (VG.time >= 400) { VG.ended = true; VG.won = true; return; }   // Timeout limit=400 win=true bonus=0.02
    if (VG.time > VG.timeoutBonusT) { VG.score += 0.02; VG.timeoutBonusT = VG.time; }
  }
  VG.tickImpl = tick;
};
