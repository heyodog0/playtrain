// vgdl/infer/aliens — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 9 types, 20 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_wall = T[1];   // wall > Immovable (inert: never updated, never leaves the screen)
  const t_base = T[2];   // base > Immovable (inert: never updated, never leaves the screen)
  const t_avatar = T[3];   // avatar > FlakAvatar
  const t_sam = T[4];   // sam > Missile
  const t_bomb = T[5];   // bomb > Missile
  const t_alien = T[6];   // alien > Bomber
  const t_portalSlow = T[7];   // portalSlow > SpawnPoint
  const t_portalFast = T[8];   // portalFast > SpawnPoint
  ...
  function tick(keys) {
    ...
    if (t_sam.jpDirty) { for (let k = 0; k < t_sam.n; k++) t_sam.jpT[t_sam.live[k]] = -1; t_sam.jpDirty = false; }
    if (t_bomb.jpDirty) { for (let k = 0; k < t_bomb.n; k++) t_bomb.jpT[t_bomb.live[k]] = -1; t_bomb.jpDirty = false; }
    if (t_alien.jpDirty) { for (let k = 0; k < t_alien.n; k++) t_alien.jpT[t_alien.live[k]] = -1; t_alien.jpDirty = false; }
    if (t_portalSlow.jpDirty) { for (let k = 0; k < t_portalSlow.n; k++) t_portalSlow.jpT[t_portalSlow.live[k]] = -1; t_portalSlow.jpDirty = false; }
    if (t_portalFast.jpDirty) { for (let k = 0; k < t_portalFast.n; k++) t_portalFast.jpT[t_portalFast.live[k]] = -1; t_portalFast.jpDirty = false; }
    const n0_avatar = t_avatar.n;
    const n0_sam = t_sam.n;
    const n0_bomb = t_bomb.n;
    const n0_alien = t_alien.n;
    const n0_portalSlow = t_portalSlow.n;
    const n0_portalFast = t_portalFast.n;
    for (let k = 0; k < n0_avatar; k++) vgUpdFlak(t_avatar, t_avatar.live[k]);   // avatar > FlakAvatar
    for (let k = 0; k < n0_sam; k++) vgBaseUpdate(t_sam, t_sam.live[k]);   // sam > Missile
    for (let k = 0; k < n0_bomb; k++) vgBaseUpdate(t_bomb, t_bomb.live[k]);   // bomb > Missile
    for (let k = 0; k < n0_alien; k++) vgUpdBomber(t_alien, t_alien.live[k]);   // alien > Bomber
    for (let k = 0; k < n0_portalSlow; k++) vgUpdSpawn(t_portalSlow, t_portalSlow.live[k]);   // portalSlow > SpawnPoint
    for (let k = 0; k < n0_portalFast; k++) vgUpdSpawn(t_portalFast, t_portalFast.live[k]);   // portalFast > SpawnPoint
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // ---- bounce pass ----
    // alien wall > turnAround
    if (t_alien.n !== 0 && t_wall.n !== 0) {
      if (t_alien.n <= t_wall.n) { for (let k = 0; k < t_alien.n; k++) { const i = t_alien.live[k]; vgCollectType(t_alien, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_alien.killed[i]) { VG_EFFECTS.turnAround(t_alien, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_alien); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_alien.killed[j]) { VG_EFFECTS.turnAround(t_alien, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // sam wall > killSprite
    if (t_sam.n !== 0 && t_wall.n !== 0) {
      if (t_sam.n <= t_wall.n) { for (let k = 0; k < t_sam.n; k++) { const i = t_sam.live[k]; vgCollectType(t_sam, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sam.killed[i]) { vgKill(t_sam, i); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_sam); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sam.killed[j]) { vgKill(t_sam, j); } } } }
    }
    // bomb wall > killSprite
    if (t_bomb.n !== 0 && t_wall.n !== 0) {
      if (t_bomb.n <= t_wall.n) { for (let k = 0; k < t_bomb.n; k++) { const i = t_bomb.live[k]; vgCollectType(t_bomb, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bomb.killed[i]) { vgKill(t_bomb, i); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_bomb); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bomb.killed[j]) { vgKill(t_bomb, j); } } } }
    }
    // base bomb > killSprite
    if (t_base.n !== 0 && t_bomb.n !== 0) {
      if (t_base.n <= t_bomb.n) { for (let k = 0; k < t_base.n; k++) { const i = t_base.live[k]; vgCollectType(t_base, i, t_bomb); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[i]) { vgKill(t_base, i); } } } }
      else { for (let k = 0; k < t_bomb.n; k++) { const i = t_bomb.live[k]; vgCollectType(t_bomb, i, t_base); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[j]) { vgKill(t_base, j); } } } }
    }
    // bomb base > killSprite
    if (t_bomb.n !== 0 && t_base.n !== 0) {
      if (t_bomb.n <= t_base.n) { for (let k = 0; k < t_bomb.n; k++) { const i = t_bomb.live[k]; vgCollectType(t_bomb, i, t_base); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bomb.killed[i]) { vgKill(t_bomb, i); } } } }
      else { for (let k = 0; k < t_base.n; k++) { const i = t_base.live[k]; vgCollectType(t_base, i, t_bomb); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bomb.killed[j]) { vgKill(t_bomb, j); } } } }
    }
    // sam base > killSprite
    if (t_sam.n !== 0 && t_base.n !== 0) {
      if (t_sam.n <= t_base.n) { for (let k = 0; k < t_sam.n; k++) { const i = t_sam.live[k]; vgCollectType(t_sam, i, t_base); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sam.killed[i]) { vgKill(t_sam, i); } } } }
      else { for (let k = 0; k < t_base.n; k++) { const i = t_base.live[k]; vgCollectType(t_base, i, t_sam); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sam.killed[j]) { vgKill(t_sam, j); } } } }
    }
    // base sam > killSprite
    if (t_base.n !== 0 && t_sam.n !== 0) {
      if (t_base.n <= t_sam.n) { for (let k = 0; k < t_base.n; k++) { const i = t_base.live[k]; vgCollectType(t_base, i, t_sam); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[i]) { vgKill(t_base, i); } } } }
      else { for (let k = 0; k < t_sam.n; k++) { const i = t_sam.live[k]; vgCollectType(t_sam, i, t_base); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[j]) { vgKill(t_base, j); } } } }
    }
    // base alien > killSprite
    if (t_base.n !== 0 && t_alien.n !== 0) {
      if (t_base.n <= t_alien.n) { for (let k = 0; k < t_base.n; k++) { const i = t_base.live[k]; vgCollectType(t_base, i, t_alien); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[i]) { vgKill(t_base, i); } } } }
      else { for (let k = 0; k < t_alien.n; k++) { const i = t_alien.live[k]; vgCollectType(t_alien, i, t_base); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_base.killed[j]) { vgKill(t_base, j); } } } }
    }
    // avatar alien > killSprite scoreChange=-1
    if (t_avatar.n !== 0 && t_alien.n !== 0) {
      if (t_avatar.n <= t_alien.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_alien); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG.score += -1; vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_alien.n; k++) { const i = t_alien.live[k]; vgCollectType(t_alien, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG.score += -1; vgKill(t_avatar, j); } } } }
    }
    // avatar bomb > killSprite scoreChange=-1
    if (t_avatar.n !== 0 && t_bomb.n !== 0) {
      if (t_avatar.n <= t_bomb.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_bomb); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG.score += -1; vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_bomb.n; k++) { const i = t_bomb.live[k]; vgCollectType(t_bomb, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG.score += -1; vgKill(t_avatar, j); } } } }
    }
    // alien sam > killSprite scoreChange=1
    if (t_alien.n !== 0 && t_sam.n !== 0) {
      if (t_alien.n <= t_sam.n) { for (let k = 0; k < t_alien.n; k++) { const i = t_alien.live[k]; vgCollectType(t_alien, i, t_sam); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_alien.killed[i]) { VG.score += 1; vgKill(t_alien, i); } } } }
      else { for (let k = 0; k < t_sam.n; k++) { const i = t_sam.live[k]; vgCollectType(t_sam, i, t_alien); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_alien.killed[j]) { VG.score += 1; vgKill(t_alien, j); } } } }
    }
    // alien EOS > killSprite
    for (let k = t_alien.n - 1; k >= 0; k--) { const i = t_alien.live[k]; if (vgContains(t_alien, i)) continue; vgKill(t_alien, i); if (!vgContains(t_alien, i)) vgKill(t_alien, i); }
    // avatar EOS > killSprite
    ...
    // bomb EOS > killSprite
    for (let k = t_bomb.n - 1; k >= 0; k--) { const i = t_bomb.live[k]; if (vgContains(t_bomb, i)) continue; vgKill(t_bomb, i); if (!vgContains(t_bomb, i)) vgKill(t_bomb, i); }
    // portalFast EOS > killSprite
    for (let k = t_portalFast.n - 1; k >= 0; k--) { const i = t_portalFast.live[k]; if (vgContains(t_portalFast, i)) continue; vgKill(t_portalFast, i); if (!vgContains(t_portalFast, i)) vgKill(t_portalFast, i); }
    // portalSlow EOS > killSprite
    for (let k = t_portalSlow.n - 1; k >= 0; k--) { const i = t_portalSlow.live[k]; if (vgContains(t_portalSlow, i)) continue; vgKill(t_portalSlow, i); if (!vgContains(t_portalSlow, i)) vgKill(t_portalSlow, i); }
    // sam EOS > killSprite
    for (let k = t_sam.n - 1; k >= 0; k--) { const i = t_sam.live[k]; if (vgContains(t_sam, i)) continue; vgKill(t_sam, i); if (!vgContains(t_sam, i)) vgKill(t_sam, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
    ...
    else if ((t_portalSlow.n + t_alien.n + t_portalFast.n) === 0) { VG.ended = true; VG.won = true; }   // MultiSpriteCounter stype1=portalSlow stype2=alien stype3=portalFast limit=0 win=true
  }
  VG.tickImpl = tick;
};
