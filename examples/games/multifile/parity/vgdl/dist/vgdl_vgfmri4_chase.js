...
const VG_GROUP_ORDER = ["wall", "angry", "avatar", "scared", "carcass"];
...
// ---- compiled tick: 5 types, 14 rules (tools/compile_vgdl.mjs, profile rcrl) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  const T = VG.types;
  const t_carcass = T[0];   // carcass > Immovable (inert)
  const t_angry = T[1];   // angry > Chaser
  const t_scared = T[2];   // scared > Fleeing
  const t_wall = T[3];   // wall > Immovable (inert)
  ...
  function events() {
    ...
    while (again) {
      const nc = new Set();
      let g_angry = null, g_scared = null, g_carcass = null, g_avatar = null, g_wall = null;   // group snapshots, one lookup per stype per pass
      // angry scared > nothing
      { const e1 = g_angry || (g_angry = rcGroupEntry("angry", cache)), l1 = e1.list;
        const e2 = g_scared || (g_scared = rcGroupEntry("scared", cache));
      ...
      // scared scared > nothing
      { const e1 = g_scared || (g_scared = rcGroupEntry("scared", cache)), l1 = e1.list;
        const e2 = g_scared || (g_scared = rcGroupEntry("scared", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
      ...
      // angry angry > nothing
      { const e1 = g_angry || (g_angry = rcGroupEntry("angry", cache)), l1 = e1.list;
        const e2 = g_angry || (g_angry = rcGroupEntry("angry", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
      ...
      // carcass avatar > nothing
      { const e1 = g_carcass || (g_carcass = rcGroupEntry("carcass", cache)), l1 = e1.list;
      ...
      // angry carcass > nothing
      { const e1 = g_angry || (g_angry = rcGroupEntry("angry", cache)), l1 = e1.list;
        const e2 = g_carcass || (g_carcass = rcGroupEntry("carcass", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
  ...
  function tick(keys) {
    ...
    const noop = keys !== null && keys.length === 0;
    VG.keys = keys || [];
    for (let k = 0; k < t_angry.n; k++) { const i = t_angry.live[k]; if (!t_angry.killed[i]) rcUpdChaser(t_angry, i); }   // angry > Chaser
    for (let k = 0; k < t_scared.n; k++) { const i = t_scared.live[k]; if (!t_scared.killed[i]) rcUpdChaser(t_scared, i); }   // scared > Fleeing
    ...
    if (rcLiveCount(t_scared) <= 0) { if (VG.time > VG.spriteBonusT) { VG.score += 5; VG.spriteBonusT = VG.time; } VG.ended = true; VG.won = true; return; }   // SpriteCounter stype=scared win=true bonus=5
    if (VG.time >= 1200) { VG.ended = true; VG.won = false; return; }   // Timeout limit=1200 win=false
    if (VG.time > VG.timeoutBonusT) { VG.score += 0; VG.timeoutBonusT = VG.time; }
  }
  VG.tickImpl = tick;
};
