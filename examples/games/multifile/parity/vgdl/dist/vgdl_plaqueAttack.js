// vgdl/infer/plaqueAttack — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 12 types, 29 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_fullMolarInf = T[1];   // fullMolarInf > Immovable (inert: never updated, never leaves the screen)
  const t_fullMolarSup = T[2];   // fullMolarSup > Immovable (inert: never updated, never leaves the screen)
  const t_deadMolarInf = T[3];   // deadMolarInf > Immovable (inert: never updated, never leaves the screen)
  const t_deadMolarSup = T[4];   // deadMolarSup > Immovable (inert: never updated, never leaves the screen)
  const t_avatar = T[5];   // avatar > ShootAvatar
  const t_hotdoghole = T[6];   // hotdoghole > SpawnPoint
  const t_burgerhole = T[7];   // burgerhole > SpawnPoint
  const t_burger = T[8];   // burger > Chaser
  const t_hotdog = T[9];   // hotdog > Chaser
  const t_fluor = T[10];   // fluor > Missile
  const t_wall = T[11];   // wall > Immovable (inert: never updated, never leaves the screen)
  const ARGS_8 = {"stype":"deadMolarInf"};
  const ARGS_10 = {"stype":"deadMolarInf"};
  const ARGS_12 = {"stype":"fullMolarInf"};
  const ARGS_13 = {"stype":"deadMolarSup"};
  const ARGS_15 = {"stype":"deadMolarSup"};
  const ARGS_17 = {"stype":"fullMolarSup"};
  ...
  function tick(keys) {
    ...
    if (t_hotdoghole.jpDirty) { for (let k = 0; k < t_hotdoghole.n; k++) t_hotdoghole.jpT[t_hotdoghole.live[k]] = -1; t_hotdoghole.jpDirty = false; }
    if (t_burgerhole.jpDirty) { for (let k = 0; k < t_burgerhole.n; k++) t_burgerhole.jpT[t_burgerhole.live[k]] = -1; t_burgerhole.jpDirty = false; }
    if (t_burger.jpDirty) { for (let k = 0; k < t_burger.n; k++) t_burger.jpT[t_burger.live[k]] = -1; t_burger.jpDirty = false; }
    if (t_hotdog.jpDirty) { for (let k = 0; k < t_hotdog.n; k++) t_hotdog.jpT[t_hotdog.live[k]] = -1; t_hotdog.jpDirty = false; }
    if (t_fluor.jpDirty) { for (let k = 0; k < t_fluor.n; k++) t_fluor.jpT[t_fluor.live[k]] = -1; t_fluor.jpDirty = false; }
    const n0_avatar = t_avatar.n;
    const n0_hotdoghole = t_hotdoghole.n;
    const n0_burgerhole = t_burgerhole.n;
    const n0_burger = t_burger.n;
    const n0_hotdog = t_hotdog.n;
    const n0_fluor = t_fluor.n;
    for (let k = 0; k < n0_avatar; k++) vgUpdShoot(t_avatar, t_avatar.live[k]);   // avatar > ShootAvatar
    for (let k = 0; k < n0_hotdoghole; k++) vgUpdSpawn(t_hotdoghole, t_hotdoghole.live[k]);   // hotdoghole > SpawnPoint
    for (let k = 0; k < n0_burgerhole; k++) vgUpdSpawn(t_burgerhole, t_burgerhole.live[k]);   // burgerhole > SpawnPoint
    for (let k = 0; k < n0_burger; k++) vgUpdChaser(t_burger, t_burger.live[k]);   // burger > Chaser
    for (let k = 0; k < n0_hotdog; k++) vgUpdChaser(t_hotdog, t_hotdog.live[k]);   // hotdog > Chaser
    for (let k = 0; k < n0_fluor; k++) vgBaseUpdate(t_fluor, t_fluor.live[k]);   // fluor > Missile
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // hotdog wall > stepBack
    if (t_hotdog.n !== 0 && t_wall.n !== 0) {
      if (t_hotdog.n <= t_wall.n) { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[i]) { VG_EFFECTS.stepBack(t_hotdog, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[j]) { VG_EFFECTS.stepBack(t_hotdog, j, t_wall, i, undefined); } } } }
    }
    // burger wall > stepBack
    if (t_burger.n !== 0 && t_wall.n !== 0) {
      if (t_burger.n <= t_wall.n) { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[i]) { VG_EFFECTS.stepBack(t_burger, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[j]) { VG_EFFECTS.stepBack(t_burger, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // fluor hotdog > killSprite
    if (t_fluor.n !== 0 && t_hotdog.n !== 0) {
      if (t_fluor.n <= t_hotdog.n) { for (let k = 0; k < t_fluor.n; k++) { const i = t_fluor.live[k]; vgCollectType(t_fluor, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[i]) { vgKill(t_fluor, i); } } } }
      else { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_fluor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[j]) { vgKill(t_fluor, j); } } } }
    }
    // hotdog fluor > killSprite scoreChange=1
    if (t_hotdog.n !== 0 && t_fluor.n !== 0) {
      if (t_hotdog.n <= t_fluor.n) { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_fluor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[i]) { VG.score += 1; vgKill(t_hotdog, i); } } } }
      else { for (let k = 0; k < t_fluor.n; k++) { const i = t_fluor.live[k]; vgCollectType(t_fluor, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[j]) { VG.score += 1; vgKill(t_hotdog, j); } } } }
    }
    // fluor burger > killSprite
    if (t_fluor.n !== 0 && t_burger.n !== 0) {
      if (t_fluor.n <= t_burger.n) { for (let k = 0; k < t_fluor.n; k++) { const i = t_fluor.live[k]; vgCollectType(t_fluor, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[i]) { vgKill(t_fluor, i); } } } }
      else { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_fluor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[j]) { vgKill(t_fluor, j); } } } }
    }
    // burger fluor > killSprite scoreChange=1
    if (t_burger.n !== 0 && t_fluor.n !== 0) {
      if (t_burger.n <= t_fluor.n) { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_fluor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[i]) { VG.score += 1; vgKill(t_burger, i); } } } }
      else { for (let k = 0; k < t_fluor.n; k++) { const i = t_fluor.live[k]; vgCollectType(t_fluor, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[j]) { VG.score += 1; vgKill(t_burger, j); } } } }
    }
    // fluor wall > killSprite
    if (t_fluor.n !== 0 && t_wall.n !== 0) {
      if (t_fluor.n <= t_wall.n) { for (let k = 0; k < t_fluor.n; k++) { const i = t_fluor.live[k]; vgCollectType(t_fluor, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[i]) { vgKill(t_fluor, i); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_fluor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fluor.killed[j]) { vgKill(t_fluor, j); } } } }
    }
    // fullMolarInf hotdog > transformTo stype=deadMolarInf scoreChange=-1
    if (t_fullMolarInf.n !== 0 && t_hotdog.n !== 0) {
      if (t_fullMolarInf.n <= t_hotdog.n) { for (let k = 0; k < t_fullMolarInf.n; k++) { const i = t_fullMolarInf.live[k]; vgCollectType(t_fullMolarInf, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarInf.killed[i]) { VG.score += -1; VG_EFFECTS.transformTo(t_fullMolarInf, i, t_hotdog, j, ARGS_8); } } } }
      else { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_fullMolarInf); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarInf.killed[j]) { VG.score += -1; VG_EFFECTS.transformTo(t_fullMolarInf, j, t_hotdog, i, ARGS_8); } } } }
    }
    // hotdog deadMolarInf > killSprite
    if (t_hotdog.n !== 0 && t_deadMolarInf.n !== 0) {
      if (t_hotdog.n <= t_deadMolarInf.n) { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_deadMolarInf); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[i]) { vgKill(t_hotdog, i); } } } }
      else { for (let k = 0; k < t_deadMolarInf.n; k++) { const i = t_deadMolarInf.live[k]; vgCollectType(t_deadMolarInf, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[j]) { vgKill(t_hotdog, j); } } } }
    }
    // fullMolarInf burger > transformTo stype=deadMolarInf scoreChange=-1
    if (t_fullMolarInf.n !== 0 && t_burger.n !== 0) {
      if (t_fullMolarInf.n <= t_burger.n) { for (let k = 0; k < t_fullMolarInf.n; k++) { const i = t_fullMolarInf.live[k]; vgCollectType(t_fullMolarInf, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarInf.killed[i]) { VG.score += -1; VG_EFFECTS.transformTo(t_fullMolarInf, i, t_burger, j, ARGS_10); } } } }
      else { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_fullMolarInf); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarInf.killed[j]) { VG.score += -1; VG_EFFECTS.transformTo(t_fullMolarInf, j, t_burger, i, ARGS_10); } } } }
    }
    // burger deadMolarInf > killSprite
    if (t_burger.n !== 0 && t_deadMolarInf.n !== 0) {
      if (t_burger.n <= t_deadMolarInf.n) { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_deadMolarInf); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[i]) { vgKill(t_burger, i); } } } }
      else { for (let k = 0; k < t_deadMolarInf.n; k++) { const i = t_deadMolarInf.live[k]; vgCollectType(t_deadMolarInf, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[j]) { vgKill(t_burger, j); } } } }
    }
    // deadMolarInf avatar > transformTo stype=fullMolarInf scoreChange=1
    if (t_deadMolarInf.n !== 0 && t_avatar.n !== 0) {
      if (t_deadMolarInf.n <= t_avatar.n) { for (let k = 0; k < t_deadMolarInf.n; k++) { const i = t_deadMolarInf.live[k]; vgCollectType(t_deadMolarInf, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_deadMolarInf.killed[i]) { VG.score += 1; VG_EFFECTS.transformTo(t_deadMolarInf, i, t_avatar, j, ARGS_12); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_deadMolarInf); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_deadMolarInf.killed[j]) { VG.score += 1; VG_EFFECTS.transformTo(t_deadMolarInf, j, t_avatar, i, ARGS_12); } } } }
    }
    // fullMolarSup hotdog > transformTo stype=deadMolarSup
    if (t_fullMolarSup.n !== 0 && t_hotdog.n !== 0) {
      if (t_fullMolarSup.n <= t_hotdog.n) { for (let k = 0; k < t_fullMolarSup.n; k++) { const i = t_fullMolarSup.live[k]; vgCollectType(t_fullMolarSup, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarSup.killed[i]) { VG_EFFECTS.transformTo(t_fullMolarSup, i, t_hotdog, j, ARGS_13); } } } }
      else { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_fullMolarSup); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarSup.killed[j]) { VG_EFFECTS.transformTo(t_fullMolarSup, j, t_hotdog, i, ARGS_13); } } } }
    }
    // hotdog deadMolarSup > killSprite scoreChange=-1
    if (t_hotdog.n !== 0 && t_deadMolarSup.n !== 0) {
      if (t_hotdog.n <= t_deadMolarSup.n) { for (let k = 0; k < t_hotdog.n; k++) { const i = t_hotdog.live[k]; vgCollectType(t_hotdog, i, t_deadMolarSup); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[i]) { VG.score += -1; vgKill(t_hotdog, i); } } } }
      else { for (let k = 0; k < t_deadMolarSup.n; k++) { const i = t_deadMolarSup.live[k]; vgCollectType(t_deadMolarSup, i, t_hotdog); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_hotdog.killed[j]) { VG.score += -1; vgKill(t_hotdog, j); } } } }
    }
    // fullMolarSup burger > transformTo stype=deadMolarSup
    if (t_fullMolarSup.n !== 0 && t_burger.n !== 0) {
      if (t_fullMolarSup.n <= t_burger.n) { for (let k = 0; k < t_fullMolarSup.n; k++) { const i = t_fullMolarSup.live[k]; vgCollectType(t_fullMolarSup, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarSup.killed[i]) { VG_EFFECTS.transformTo(t_fullMolarSup, i, t_burger, j, ARGS_15); } } } }
      else { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_fullMolarSup); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fullMolarSup.killed[j]) { VG_EFFECTS.transformTo(t_fullMolarSup, j, t_burger, i, ARGS_15); } } } }
    }
    // burger deadMolarSup > killSprite scoreChange=-1
    if (t_burger.n !== 0 && t_deadMolarSup.n !== 0) {
      if (t_burger.n <= t_deadMolarSup.n) { for (let k = 0; k < t_burger.n; k++) { const i = t_burger.live[k]; vgCollectType(t_burger, i, t_deadMolarSup); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[i]) { VG.score += -1; vgKill(t_burger, i); } } } }
      else { for (let k = 0; k < t_deadMolarSup.n; k++) { const i = t_deadMolarSup.live[k]; vgCollectType(t_deadMolarSup, i, t_burger); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_burger.killed[j]) { VG.score += -1; vgKill(t_burger, j); } } } }
    }
    // deadMolarSup avatar > transformTo stype=fullMolarSup scoreChange=1
    if (t_deadMolarSup.n !== 0 && t_avatar.n !== 0) {
      if (t_deadMolarSup.n <= t_avatar.n) { for (let k = 0; k < t_deadMolarSup.n; k++) { const i = t_deadMolarSup.live[k]; vgCollectType(t_deadMolarSup, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_deadMolarSup.killed[i]) { VG.score += 1; VG_EFFECTS.transformTo(t_deadMolarSup, i, t_avatar, j, ARGS_17); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_deadMolarSup); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_deadMolarSup.killed[j]) { VG.score += 1; VG_EFFECTS.transformTo(t_deadMolarSup, j, t_avatar, i, ARGS_17); } } } }
    }
    // avatar EOS > killSprite
    ...
    // burger EOS > killSprite
    for (let k = t_burger.n - 1; k >= 0; k--) { const i = t_burger.live[k]; if (vgContains(t_burger, i)) continue; vgKill(t_burger, i); if (!vgContains(t_burger, i)) vgKill(t_burger, i); }
    // burgerhole EOS > killSprite
    for (let k = t_burgerhole.n - 1; k >= 0; k--) { const i = t_burgerhole.live[k]; if (vgContains(t_burgerhole, i)) continue; vgKill(t_burgerhole, i); if (!vgContains(t_burgerhole, i)) vgKill(t_burgerhole, i); }
    // deadMolarInf EOS > killSprite
    // (static type: never leaves the screen)
    // deadMolarSup EOS > killSprite
    // (static type: never leaves the screen)
    // fluor EOS > killSprite
    for (let k = t_fluor.n - 1; k >= 0; k--) { const i = t_fluor.live[k]; if (vgContains(t_fluor, i)) continue; vgKill(t_fluor, i); if (!vgContains(t_fluor, i)) vgKill(t_fluor, i); }
    // fullMolarInf EOS > killSprite
    // (static type: never leaves the screen)
    // fullMolarSup EOS > killSprite
    // (static type: never leaves the screen)
    // hotdog EOS > killSprite
    for (let k = t_hotdog.n - 1; k >= 0; k--) { const i = t_hotdog.live[k]; if (vgContains(t_hotdog, i)) continue; vgKill(t_hotdog, i); if (!vgContains(t_hotdog, i)) vgKill(t_hotdog, i); }
    // hotdoghole EOS > killSprite
    for (let k = t_hotdoghole.n - 1; k >= 0; k--) { const i = t_hotdoghole.live[k]; if (vgContains(t_hotdoghole, i)) continue; vgKill(t_hotdoghole, i); if (!vgContains(t_hotdoghole, i)) vgKill(t_hotdoghole, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
    ...
    VG.ended = false; VG.won = false;
    if (VG.time >= 3000) { VG.ended = true; VG.won = false; }   // Timeout limit=3000 win=false
    else if ((t_fullMolarInf.n + t_fullMolarSup.n) === 0) { VG.ended = true; VG.won = false; }   // MultiSpriteCounter stype1=fullMolarInf stype2=fullMolarSup limit=0 win=false
    else if ((t_hotdoghole.n + t_hotdog.n + t_burger.n + t_burgerhole.n) === 0) { VG.ended = true; VG.won = true; }   // MultiSpriteCounter stype1=hotdoghole stype2=hotdog stype3=burger stype4=burgerhole limit=0 win=true
  }
  VG.tickImpl = tick;
};
