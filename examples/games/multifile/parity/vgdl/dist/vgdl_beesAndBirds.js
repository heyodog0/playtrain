// vgdl/infer/beesAndBirds — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
const VG_LEVELS = ["wwwwwwwwwwwwwwww\nw.....A........w\nw..............w\nw..........b...w\nw...b..........w\nw..............w\nw...b..........w\nw..............w\nwwwwwwwwww...www\nw..............w\nw..............w\nw..b...........w\nw.........d....w\nw.........g....w\nwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwww\nw.....A...fo1.ow\nw.........fo..ow\nw.........fffffw\nw..............w\nw....b.........w\nw.b............w\nw..............w\nwwwwwwwwwwooowww\nw..d...........w\nw..............w\nwb.....b.......w\nw..............w\nw.........g....w\nwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwww\nwwwwwwwwwwwwwwww\nw.......f...1..w\nw.......f......w\nw.......f......w\nw.......wwwwfffw\nw....wwwwwww...w\nw........A.w...w\nwfffwwwwwwwwooow\nwd..b.b.bf.....w\nwb..b..bbf....gw\nwwwwwwwwwwwwwwww\nwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwww\nw..f..A........w\nw.1f........fffw\nwfff........f1.w\nw...........fffw\nw..oooooooooooow\nw..obb..b.b..b.w\nw..o.b..b.b.b..w\nw..ob.b..b.b.b.w\nw..o.b.d...b.b.w\nw..oooooooooooow\nw.........g....w\nwwwwwwwwwwwwwwww\n"];
const VG_BLOCK = 50;
...
// ---- compiled tick: 9 types, 25 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_bee = T[2];   // bee > RandomNPC
  const t_distractor = T[3];   // distractor > Immovable (inert: never updated, never leaves the screen)
  const t_fence = T[4];   // fence > Immovable (inert: never updated, never leaves the screen)
  const t_goal = T[5];   // goal > Immovable (inert: never updated, never leaves the screen)
  const t_obstacle = T[6];   // obstacle > Immovable (inert: never updated, never leaves the screen)
  const t_sparrow1 = T[7];   // sparrow1 > Chaser
  const t_wall = T[8];   // wall > Immovable (inert: never updated, never leaves the screen)
  ...
  function tick(keys) {
    ...
    if (t_bee.jpDirty) { for (let k = 0; k < t_bee.n; k++) t_bee.jpT[t_bee.live[k]] = -1; t_bee.jpDirty = false; }
    if (t_sparrow1.jpDirty) { for (let k = 0; k < t_sparrow1.n; k++) t_sparrow1.jpT[t_sparrow1.live[k]] = -1; t_sparrow1.jpDirty = false; }
    const n0_avatar = t_avatar.n;
    const n0_bee = t_bee.n;
    const n0_sparrow1 = t_sparrow1.n;
    for (let k = 0; k < n0_avatar; k++) vgUpdMoving(t_avatar, t_avatar.live[k]);   // avatar > MovingAvatar
    for (let k = 0; k < n0_bee; k++) vgUpdRandom(t_bee, t_bee.live[k]);   // bee > RandomNPC
    for (let k = 0; k < n0_sparrow1; k++) vgUpdChaser(t_sparrow1, t_sparrow1.live[k]);   // sparrow1 > Chaser
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // bee distractor > stepBack
    if (t_bee.n !== 0 && t_distractor.n !== 0) {
      if (t_bee.n <= t_distractor.n) { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_distractor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[i]) { VG_EFFECTS.stepBack(t_bee, i, t_distractor, j, undefined); } } } }
      else { for (let k = 0; k < t_distractor.n; k++) { const i = t_distractor.live[k]; vgCollectType(t_distractor, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[j]) { VG_EFFECTS.stepBack(t_bee, j, t_distractor, i, undefined); } } } }
    }
    // bee fence > stepBack
    if (t_bee.n !== 0 && t_fence.n !== 0) {
      if (t_bee.n <= t_fence.n) { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_fence); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[i]) { VG_EFFECTS.stepBack(t_bee, i, t_fence, j, undefined); } } } }
      else { for (let k = 0; k < t_fence.n; k++) { const i = t_fence.live[k]; vgCollectType(t_fence, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[j]) { VG_EFFECTS.stepBack(t_bee, j, t_fence, i, undefined); } } } }
    }
    // bee obstacle > stepBack
    if (t_bee.n !== 0 && t_obstacle.n !== 0) {
      if (t_bee.n <= t_obstacle.n) { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_obstacle); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[i]) { VG_EFFECTS.stepBack(t_bee, i, t_obstacle, j, undefined); } } } }
      else { for (let k = 0; k < t_obstacle.n; k++) { const i = t_obstacle.live[k]; vgCollectType(t_obstacle, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[j]) { VG_EFFECTS.stepBack(t_bee, j, t_obstacle, i, undefined); } } } }
    }
    // bee wall > stepBack
    if (t_bee.n !== 0 && t_wall.n !== 0) {
      if (t_bee.n <= t_wall.n) { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[i]) { VG_EFFECTS.stepBack(t_bee, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[j]) { VG_EFFECTS.stepBack(t_bee, j, t_wall, i, undefined); } } } }
    }
    // sparrow1 fence > stepBack
    if (t_sparrow1.n !== 0 && t_fence.n !== 0) {
      if (t_sparrow1.n <= t_fence.n) { for (let k = 0; k < t_sparrow1.n; k++) { const i = t_sparrow1.live[k]; vgCollectType(t_sparrow1, i, t_fence); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sparrow1.killed[i]) { VG_EFFECTS.stepBack(t_sparrow1, i, t_fence, j, undefined); } } } }
      else { for (let k = 0; k < t_fence.n; k++) { const i = t_fence.live[k]; vgCollectType(t_fence, i, t_sparrow1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sparrow1.killed[j]) { VG_EFFECTS.stepBack(t_sparrow1, j, t_fence, i, undefined); } } } }
    }
    // sparrow1 wall > stepBack
    if (t_sparrow1.n !== 0 && t_wall.n !== 0) {
      if (t_sparrow1.n <= t_wall.n) { for (let k = 0; k < t_sparrow1.n; k++) { const i = t_sparrow1.live[k]; vgCollectType(t_sparrow1, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sparrow1.killed[i]) { VG_EFFECTS.stepBack(t_sparrow1, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_sparrow1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sparrow1.killed[j]) { VG_EFFECTS.stepBack(t_sparrow1, j, t_wall, i, undefined); } } } }
    }
    // bee EOS > stepBack
    for (let k = t_bee.n - 1; k >= 0; k--) { const i = t_bee.live[k]; if (vgContains(t_bee, i)) continue; VG_EFFECTS.stepBack(t_bee, i, null, -1, undefined); if (!vgContains(t_bee, i)) vgKill(t_bee, i); }
    // sparrow1 EOS > stepBack
    for (let k = t_sparrow1.n - 1; k >= 0; k--) { const i = t_sparrow1.live[k]; if (vgContains(t_sparrow1, i)) continue; VG_EFFECTS.stepBack(t_sparrow1, i, null, -1, undefined); if (!vgContains(t_sparrow1, i)) vgKill(t_sparrow1, i); }
    // avatar EOS > stepBack
    for (let k = t_avatar.n - 1; k >= 0; k--) { const i = t_avatar.live[k]; if (vgContains(t_avatar, i)) continue; VG_EFFECTS.stepBack(t_avatar, i, null, -1, undefined); if (!vgContains(t_avatar, i)) vgKill(t_avatar, i); }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
      if (t_avatar.n <= t_wall.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_wall); const nc = vgCand.length;
    ...
    // ---- non-move effects ----
    // avatar bee > killSprite
    if (t_avatar.n !== 0 && t_bee.n !== 0) {
      if (t_avatar.n <= t_bee.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar obstacle > killSprite
    if (t_avatar.n !== 0 && t_obstacle.n !== 0) {
      if (t_avatar.n <= t_obstacle.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_obstacle); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_obstacle.n; k++) { const i = t_obstacle.live[k]; vgCollectType(t_obstacle, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // bee sparrow1 > killSprite
    if (t_bee.n !== 0 && t_sparrow1.n !== 0) {
      if (t_bee.n <= t_sparrow1.n) { for (let k = 0; k < t_bee.n; k++) { const i = t_bee.live[k]; vgCollectType(t_bee, i, t_sparrow1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[i]) { vgKill(t_bee, i); } } } }
      else { for (let k = 0; k < t_sparrow1.n; k++) { const i = t_sparrow1.live[k]; vgCollectType(t_sparrow1, i, t_bee); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_bee.killed[j]) { vgKill(t_bee, j); } } } }
    }
    // distractor avatar > killSprite
    if (t_distractor.n !== 0 && t_avatar.n !== 0) {
      if (t_distractor.n <= t_avatar.n) { for (let k = 0; k < t_distractor.n; k++) { const i = t_distractor.live[k]; vgCollectType(t_distractor, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_distractor.killed[i]) { vgKill(t_distractor, i); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_distractor); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_distractor.killed[j]) { vgKill(t_distractor, j); } } } }
    }
    // fence avatar > killSprite
    if (t_fence.n !== 0 && t_avatar.n !== 0) {
      if (t_fence.n <= t_avatar.n) { for (let k = 0; k < t_fence.n; k++) { const i = t_fence.live[k]; vgCollectType(t_fence, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fence.killed[i]) { vgKill(t_fence, i); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_fence); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fence.killed[j]) { vgKill(t_fence, j); } } } }
    }
    // goal avatar > killSprite scoreChange=1
    ...
    // obstacle sparrow1 > killSprite
    if (t_obstacle.n !== 0 && t_sparrow1.n !== 0) {
      if (t_obstacle.n <= t_sparrow1.n) { for (let k = 0; k < t_obstacle.n; k++) { const i = t_obstacle.live[k]; vgCollectType(t_obstacle, i, t_sparrow1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_obstacle.killed[i]) { vgKill(t_obstacle, i); } } } }
      else { for (let k = 0; k < t_sparrow1.n; k++) { const i = t_sparrow1.live[k]; vgCollectType(t_sparrow1, i, t_obstacle); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_obstacle.killed[j]) { vgKill(t_obstacle, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // bee EOS > killSprite
    for (let k = t_bee.n - 1; k >= 0; k--) { const i = t_bee.live[k]; if (vgContains(t_bee, i)) continue; vgKill(t_bee, i); if (!vgContains(t_bee, i)) vgKill(t_bee, i); }
    // distractor EOS > killSprite
    ...
    // (static type: never leaves the screen)
    // sparrow1 EOS > killSprite
    for (let k = t_sparrow1.n - 1; k >= 0; k--) { const i = t_sparrow1.live[k]; if (vgContains(t_sparrow1, i)) continue; vgKill(t_sparrow1, i); if (!vgContains(t_sparrow1, i)) vgKill(t_sparrow1, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
