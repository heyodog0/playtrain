// vgdl/infer/relational — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
const VG_LEVELS = ["wwwwwwwwwwwwwwwwwwwwww\nwfA.......w.........fw\nw....a....x..........w\nw..............f.....w\nw.w.........w........w\nw......f.............w\nw..............w..x..w\nw...w......a.........w\nwf..................fw\nwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwww\nwA........w..........w\nw....a.w..x...p......w\nw................w...w\nw....w.....w.........w\nw...w..........e.....w\nw...x....e.......w...w\nw..........a.....p...w\nw....................w\nwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwww\nw.........w..........w\nw.w..a....x....p.....w\nw....................w\nw..p..w....w.........w\nw..............y.....w\nw...x....y...........w\nw.....w....a....w....w\nw..........A.........w\nwwwwwwwwwwwwwwwwwwwwww\n","wwwwwwwwwwwwwwwwwwwwww\nw.........w..........w\nw.....p...x..........w\nw............y..w....w\nw....w.p.............w\nw..........y...z.....w\nw...x....z...........w\nw..............w.....w\nw..........A.........w\nwwwwwwwwwwwwwwwwwwwwww\n"];
const VG_BLOCK = 50;
...
// ---- compiled tick: 10 types, 36 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_box = T[2];   // box > Passive
  const t_converter1 = T[3];   // converter1 > Passive
  const t_converter2 = T[4];   // converter2 > Immovable (inert: never updated, never leaves the screen)
  const t_converter3 = T[5];   // converter3 > Immovable (inert: never updated, never leaves the screen)
  const t_fire = T[6];   // fire > Immovable (inert: never updated, never leaves the screen)
  const t_poison = T[7];   // poison > Immovable (inert: never updated, never leaves the screen)
  const t_probe = T[8];   // probe > Passive
  const t_wall = T[9];   // wall > Immovable (inert: never updated, never leaves the screen)
  const ARGS_5 = {"stype":"fire"};
  const ARGS_15 = {"stype":"fire"};
  const ARGS_18 = {"stype":"box"};
  ...
  function tick(keys) {
    ...
    if (t_box.jpDirty) { for (let k = 0; k < t_box.n; k++) t_box.jpT[t_box.live[k]] = -1; t_box.jpDirty = false; }
    if (t_converter1.jpDirty) { for (let k = 0; k < t_converter1.n; k++) t_converter1.jpT[t_converter1.live[k]] = -1; t_converter1.jpDirty = false; }
    if (t_probe.jpDirty) { for (let k = 0; k < t_probe.n; k++) t_probe.jpT[t_probe.live[k]] = -1; t_probe.jpDirty = false; }
    const n0_avatar = t_avatar.n;
    const n0_box = t_box.n;
    const n0_converter1 = t_converter1.n;
    const n0_probe = t_probe.n;
    for (let k = 0; k < n0_avatar; k++) vgUpdMoving(t_avatar, t_avatar.live[k]);   // avatar > MovingAvatar
    for (let k = 0; k < n0_box; k++) vgBaseUpdate(t_box, t_box.live[k]);   // box > Passive
    for (let k = 0; k < n0_converter1; k++) vgBaseUpdate(t_converter1, t_converter1.live[k]);   // converter1 > Passive
    for (let k = 0; k < n0_probe; k++) vgBaseUpdate(t_probe, t_probe.live[k]);   // probe > Passive
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar fire > stepBack
    if (t_avatar.n !== 0 && t_fire.n !== 0) {
      if (t_avatar.n <= t_fire.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_fire); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG_EFFECTS.stepBack(t_avatar, i, t_fire, j, undefined); } } } }
      else { for (let k = 0; k < t_fire.n; k++) { const i = t_fire.live[k]; vgCollectType(t_fire, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG_EFFECTS.stepBack(t_avatar, j, t_fire, i, undefined); } } } }
    }
    // avatar wall > stepBack
    ...
    // box box > stepBack
    if (t_box.n > 1) for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_box); const nc = vgCand.length;
      for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (j === i) continue; if (!t_box.killed[i]) { VG_EFFECTS.stepBack(t_box, i, t_box, j, undefined); } } }
    // box fire > stepBack
    if (t_box.n !== 0 && t_fire.n !== 0) {
      if (t_box.n <= t_fire.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_fire); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.stepBack(t_box, i, t_fire, j, undefined); } } } }
      else { for (let k = 0; k < t_fire.n; k++) { const i = t_fire.live[k]; vgCollectType(t_fire, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.stepBack(t_box, j, t_fire, i, undefined); } } } }
    }
    // box probe > stepBack
    if (t_box.n !== 0 && t_probe.n !== 0) {
      if (t_box.n <= t_probe.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.stepBack(t_box, i, t_probe, j, undefined); } } } }
      else { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.stepBack(t_box, j, t_probe, i, undefined); } } } }
    }
    // box wall > stepBack
    if (t_box.n !== 0 && t_wall.n !== 0) {
      if (t_box.n <= t_wall.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.stepBack(t_box, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.stepBack(t_box, j, t_wall, i, undefined); } } } }
    }
    // converter1 poison > stepBack
    if (t_converter1.n !== 0 && t_poison.n !== 0) {
      if (t_converter1.n <= t_poison.n) { for (let k = 0; k < t_converter1.n; k++) { const i = t_converter1.live[k]; vgCollectType(t_converter1, i, t_poison); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[i]) { VG_EFFECTS.stepBack(t_converter1, i, t_poison, j, undefined); } } } }
      else { for (let k = 0; k < t_poison.n; k++) { const i = t_poison.live[k]; vgCollectType(t_poison, i, t_converter1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[j]) { VG_EFFECTS.stepBack(t_converter1, j, t_poison, i, undefined); } } } }
    }
    // box poison > stepBack
    if (t_box.n !== 0 && t_poison.n !== 0) {
      if (t_box.n <= t_poison.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_poison); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.stepBack(t_box, i, t_poison, j, undefined); } } } }
      else { for (let k = 0; k < t_poison.n; k++) { const i = t_poison.live[k]; vgCollectType(t_poison, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.stepBack(t_box, j, t_poison, i, undefined); } } } }
    }
    // probe poison > stepBack
    if (t_probe.n !== 0 && t_poison.n !== 0) {
      if (t_probe.n <= t_poison.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_poison); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_poison, j, undefined); } } } }
      else { for (let k = 0; k < t_poison.n; k++) { const i = t_poison.live[k]; vgCollectType(t_poison, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_poison, i, undefined); } } } }
    }
    // converter1 wall > stepBack
    if (t_converter1.n !== 0 && t_wall.n !== 0) {
      if (t_converter1.n <= t_wall.n) { for (let k = 0; k < t_converter1.n; k++) { const i = t_converter1.live[k]; vgCollectType(t_converter1, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[i]) { VG_EFFECTS.stepBack(t_converter1, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_converter1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[j]) { VG_EFFECTS.stepBack(t_converter1, j, t_wall, i, undefined); } } } }
    }
    // converter2 wall > stepBack
    if (t_converter2.n !== 0 && t_wall.n !== 0) {
      if (t_converter2.n <= t_wall.n) { for (let k = 0; k < t_converter2.n; k++) { const i = t_converter2.live[k]; vgCollectType(t_converter2, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter2.killed[i]) { VG_EFFECTS.stepBack(t_converter2, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_converter2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter2.killed[j]) { VG_EFFECTS.stepBack(t_converter2, j, t_wall, i, undefined); } } } }
    }
    // converter3 wall > stepBack
    if (t_converter3.n !== 0 && t_wall.n !== 0) {
      if (t_converter3.n <= t_wall.n) { for (let k = 0; k < t_converter3.n; k++) { const i = t_converter3.live[k]; vgCollectType(t_converter3, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter3.killed[i]) { VG_EFFECTS.stepBack(t_converter3, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_converter3); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter3.killed[j]) { VG_EFFECTS.stepBack(t_converter3, j, t_wall, i, undefined); } } } }
    }
    // probe box > stepBack
    if (t_probe.n !== 0 && t_box.n !== 0) {
      if (t_probe.n <= t_box.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_box, j, undefined); } } } }
      else { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_box, i, undefined); } } } }
    }
    // probe converter1 > stepBack
    if (t_probe.n !== 0 && t_converter1.n !== 0) {
      if (t_probe.n <= t_converter1.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_converter1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_converter1, j, undefined); } } } }
      else { for (let k = 0; k < t_converter1.n; k++) { const i = t_converter1.live[k]; vgCollectType(t_converter1, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_converter1, i, undefined); } } } }
    }
    // probe converter2 > stepBack
    if (t_probe.n !== 0 && t_converter2.n !== 0) {
      if (t_probe.n <= t_converter2.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_converter2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_converter2, j, undefined); } } } }
      else { for (let k = 0; k < t_converter2.n; k++) { const i = t_converter2.live[k]; vgCollectType(t_converter2, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_converter2, i, undefined); } } } }
    }
    // probe converter3 > stepBack
    if (t_probe.n !== 0 && t_converter3.n !== 0) {
      if (t_probe.n <= t_converter3.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_converter3); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_converter3, j, undefined); } } } }
      else { for (let k = 0; k < t_converter3.n; k++) { const i = t_converter3.live[k]; vgCollectType(t_converter3, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_converter3, i, undefined); } } } }
    }
    // probe probe > stepBack
    if (t_probe.n > 1) for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_probe); const nc = vgCand.length;
      for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (j === i) continue; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_probe, j, undefined); } } }
    // probe wall > stepBack
    if (t_probe.n !== 0 && t_wall.n !== 0) {
      if (t_probe.n <= t_wall.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.stepBack(t_probe, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.stepBack(t_probe, j, t_wall, i, undefined); } } } }
    }
    // ---- bounce pass ----
    // box avatar > bounceForward
    if (t_box.n !== 0 && t_avatar.n !== 0) {
      if (t_box.n <= t_avatar.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.bounceForward(t_box, i, t_avatar, j, undefined); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.bounceForward(t_box, j, t_avatar, i, undefined); } } } }
    }
    // converter1 box > bounceForward
    if (t_converter1.n !== 0 && t_box.n !== 0) {
      if (t_converter1.n <= t_box.n) { for (let k = 0; k < t_converter1.n; k++) { const i = t_converter1.live[k]; vgCollectType(t_converter1, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[i]) { VG_EFFECTS.bounceForward(t_converter1, i, t_box, j, undefined); } } } }
      else { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_converter1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[j]) { VG_EFFECTS.bounceForward(t_converter1, j, t_box, i, undefined); } } } }
    }
    // probe avatar > bounceForward
    if (t_probe.n !== 0 && t_avatar.n !== 0) {
      if (t_probe.n <= t_avatar.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { VG_EFFECTS.bounceForward(t_probe, i, t_avatar, j, undefined); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { VG_EFFECTS.bounceForward(t_probe, j, t_avatar, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar fire > stepBack
    if (t_avatar.n !== 0 && t_fire.n !== 0) {
    ...
    if (t_avatar.n !== 0 && t_poison.n !== 0) {
      if (t_avatar.n <= t_poison.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_poison); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_poison.n; k++) { const i = t_poison.live[k]; vgCollectType(t_poison, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // box converter2 > transformTo stype=fire
    if (t_box.n !== 0 && t_converter2.n !== 0) {
      if (t_box.n <= t_converter2.n) { for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; vgCollectType(t_box, i, t_converter2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[i]) { VG_EFFECTS.transformTo(t_box, i, t_converter2, j, ARGS_5); } } } }
      else { for (let k = 0; k < t_converter2.n; k++) { const i = t_converter2.live[k]; vgCollectType(t_converter2, i, t_box); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box.killed[j]) { VG_EFFECTS.transformTo(t_box, j, t_converter2, i, ARGS_5); } } } }
    }
    // converter1 avatar > transformTo stype=fire
    if (t_converter1.n !== 0 && t_avatar.n !== 0) {
      if (t_converter1.n <= t_avatar.n) { for (let k = 0; k < t_converter1.n; k++) { const i = t_converter1.live[k]; vgCollectType(t_converter1, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[i]) { VG_EFFECTS.transformTo(t_converter1, i, t_avatar, j, ARGS_15); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_converter1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter1.killed[j]) { VG_EFFECTS.transformTo(t_converter1, j, t_avatar, i, ARGS_15); } } } }
    }
    // converter2 fire > killSprite
    if (t_converter2.n !== 0 && t_fire.n !== 0) {
      if (t_converter2.n <= t_fire.n) { for (let k = 0; k < t_converter2.n; k++) { const i = t_converter2.live[k]; vgCollectType(t_converter2, i, t_fire); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter2.killed[i]) { vgKill(t_converter2, i); } } } }
      else { for (let k = 0; k < t_fire.n; k++) { const i = t_fire.live[k]; vgCollectType(t_fire, i, t_converter2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter2.killed[j]) { vgKill(t_converter2, j); } } } }
    }
    // converter3 avatar > transformTo stype=box
    if (t_converter3.n !== 0 && t_avatar.n !== 0) {
      if (t_converter3.n <= t_avatar.n) { for (let k = 0; k < t_converter3.n; k++) { const i = t_converter3.live[k]; vgCollectType(t_converter3, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter3.killed[i]) { VG_EFFECTS.transformTo(t_converter3, i, t_avatar, j, ARGS_18); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_converter3); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_converter3.killed[j]) { VG_EFFECTS.transformTo(t_converter3, j, t_avatar, i, ARGS_18); } } } }
    }
    // fire probe > killSprite scoreChange=1
    if (t_fire.n !== 0 && t_probe.n !== 0) {
      if (t_fire.n <= t_probe.n) { for (let k = 0; k < t_fire.n; k++) { const i = t_fire.live[k]; vgCollectType(t_fire, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fire.killed[i]) { VG.score += 1; vgKill(t_fire, i); } } } }
      else { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_fire); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_fire.killed[j]) { VG.score += 1; vgKill(t_fire, j); } } } }
    }
    // probe fire > killSprite
    if (t_probe.n !== 0 && t_fire.n !== 0) {
      if (t_probe.n <= t_fire.n) { for (let k = 0; k < t_probe.n; k++) { const i = t_probe.live[k]; vgCollectType(t_probe, i, t_fire); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[i]) { vgKill(t_probe, i); } } } }
      else { for (let k = 0; k < t_fire.n; k++) { const i = t_fire.live[k]; vgCollectType(t_fire, i, t_probe); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_probe.killed[j]) { vgKill(t_probe, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // box EOS > killSprite
    for (let k = t_box.n - 1; k >= 0; k--) { const i = t_box.live[k]; if (vgContains(t_box, i)) continue; vgKill(t_box, i); if (!vgContains(t_box, i)) vgKill(t_box, i); }
    // converter1 EOS > killSprite
    for (let k = t_converter1.n - 1; k >= 0; k--) { const i = t_converter1.live[k]; if (vgContains(t_converter1, i)) continue; vgKill(t_converter1, i); if (!vgContains(t_converter1, i)) vgKill(t_converter1, i); }
    // converter2 EOS > killSprite
    ...
    // (static type: never leaves the screen)
    // probe EOS > killSprite
    for (let k = t_probe.n - 1; k >= 0; k--) { const i = t_probe.live[k]; if (vgContains(t_probe, i)) continue; vgKill(t_probe, i); if (!vgContains(t_probe, i)) vgKill(t_probe, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
    ...
    else if (t_probe.n <= 0) { VG.ended = true; VG.won = true; }   // SpriteCounter stype=probe limit=0 win=true
