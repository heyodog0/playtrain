// vgdl/infer/portals — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 12 types, 34 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_sitting = T[1];   // sitting > Immovable (inert: never updated, never leaves the screen)
  const t_vertical = T[2];   // vertical > Missile
  const t_horizontal = T[3];   // horizontal > Missile
  const t_goal = T[4];   // goal > Immovable (inert: never updated, never leaves the screen)
  const t_random = T[5];   // random > RandomNPC
  const t_entry1 = T[6];   // entry1 > Portal (inert: never updated, never leaves the screen)
  const t_entry2 = T[7];   // entry2 > Portal (inert: never updated, never leaves the screen)
  const t_exit1 = T[8];   // exit1 > Immovable (inert: never updated, never leaves the screen)
  const t_exit2 = T[9];   // exit2 > Immovable (inert: never updated, never leaves the screen)
  const t_wall = T[10];   // wall > Immovable (inert: never updated, never leaves the screen)
  const t_avatar = T[11];   // avatar > MovingAvatar
  ...
  function tick(keys) {
    ...
    if (t_vertical.jpDirty) { for (let k = 0; k < t_vertical.n; k++) t_vertical.jpT[t_vertical.live[k]] = -1; t_vertical.jpDirty = false; }
    if (t_horizontal.jpDirty) { for (let k = 0; k < t_horizontal.n; k++) t_horizontal.jpT[t_horizontal.live[k]] = -1; t_horizontal.jpDirty = false; }
    if (t_random.jpDirty) { for (let k = 0; k < t_random.n; k++) t_random.jpT[t_random.live[k]] = -1; t_random.jpDirty = false; }
    if (t_avatar.jpDirty) { for (let k = 0; k < t_avatar.n; k++) t_avatar.jpT[t_avatar.live[k]] = -1; t_avatar.jpDirty = false; }
    const n0_vertical = t_vertical.n;
    const n0_horizontal = t_horizontal.n;
    const n0_random = t_random.n;
    const n0_avatar = t_avatar.n;
    for (let k = 0; k < n0_vertical; k++) vgBaseUpdate(t_vertical, t_vertical.live[k]);   // vertical > Missile
    for (let k = 0; k < n0_horizontal; k++) vgBaseUpdate(t_horizontal, t_horizontal.live[k]);   // horizontal > Missile
    for (let k = 0; k < n0_random; k++) vgUpdRandom(t_random, t_random.live[k]);   // random > RandomNPC
    for (let k = 0; k < n0_avatar; k++) vgUpdMoving(t_avatar, t_avatar.live[k]);   // avatar > MovingAvatar
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // random entry1 > stepBack
    if (t_random.n !== 0 && t_entry1.n !== 0) {
      if (t_random.n <= t_entry1.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_entry1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_entry1, j, undefined); } } } }
      else { for (let k = 0; k < t_entry1.n; k++) { const i = t_entry1.live[k]; vgCollectType(t_entry1, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_entry1, i, undefined); } } } }
    }
    // random entry2 > stepBack
    if (t_random.n !== 0 && t_entry2.n !== 0) {
      if (t_random.n <= t_entry2.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_entry2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_entry2, j, undefined); } } } }
      else { for (let k = 0; k < t_entry2.n; k++) { const i = t_entry2.live[k]; vgCollectType(t_entry2, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_entry2, i, undefined); } } } }
    }
    // random exit1 > stepBack
    if (t_random.n !== 0 && t_exit1.n !== 0) {
      if (t_random.n <= t_exit1.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_exit1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_exit1, j, undefined); } } } }
      else { for (let k = 0; k < t_exit1.n; k++) { const i = t_exit1.live[k]; vgCollectType(t_exit1, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_exit1, i, undefined); } } } }
    }
    // random exit2 > stepBack
    if (t_random.n !== 0 && t_exit2.n !== 0) {
      if (t_random.n <= t_exit2.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_exit2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_exit2, j, undefined); } } } }
      else { for (let k = 0; k < t_exit2.n; k++) { const i = t_exit2.live[k]; vgCollectType(t_exit2, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_exit2, i, undefined); } } } }
    }
    // random goal > stepBack
    if (t_random.n !== 0 && t_goal.n !== 0) {
      if (t_random.n <= t_goal.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_goal, j, undefined); } } } }
      else { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_goal, i, undefined); } } } }
    }
    // random wall > stepBack
    if (t_random.n !== 0 && t_wall.n !== 0) {
      if (t_random.n <= t_wall.n) { for (let k = 0; k < t_random.n; k++) { const i = t_random.live[k]; vgCollectType(t_random, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[i]) { VG_EFFECTS.stepBack(t_random, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_random); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_random.killed[j]) { VG_EFFECTS.stepBack(t_random, j, t_wall, i, undefined); } } } }
    }
    // horizontal goal > stepBack
    if (t_horizontal.n !== 0 && t_goal.n !== 0) {
      if (t_horizontal.n <= t_goal.n) { for (let k = 0; k < t_horizontal.n; k++) { const i = t_horizontal.live[k]; vgCollectType(t_horizontal, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_horizontal.killed[i]) { VG_EFFECTS.stepBack(t_horizontal, i, t_goal, j, undefined); } } } }
      else { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_horizontal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_horizontal.killed[j]) { VG_EFFECTS.stepBack(t_horizontal, j, t_goal, i, undefined); } } } }
    }
    // vertical goal > stepBack
    if (t_vertical.n !== 0 && t_goal.n !== 0) {
      if (t_vertical.n <= t_goal.n) { for (let k = 0; k < t_vertical.n; k++) { const i = t_vertical.live[k]; vgCollectType(t_vertical, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_vertical.killed[i]) { VG_EFFECTS.stepBack(t_vertical, i, t_goal, j, undefined); } } } }
      else { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_vertical); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_vertical.killed[j]) { VG_EFFECTS.stepBack(t_vertical, j, t_goal, i, undefined); } } } }
    }
    // sitting goal > stepBack
    if (t_sitting.n !== 0 && t_goal.n !== 0) {
      if (t_sitting.n <= t_goal.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_goal, j, undefined); } } } }
      else { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_goal, i, undefined); } } } }
    }
    // sitting entry1 > stepBack
    if (t_sitting.n !== 0 && t_entry1.n !== 0) {
      if (t_sitting.n <= t_entry1.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_entry1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_entry1, j, undefined); } } } }
      else { for (let k = 0; k < t_entry1.n; k++) { const i = t_entry1.live[k]; vgCollectType(t_entry1, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_entry1, i, undefined); } } } }
    }
    // sitting entry2 > stepBack
    if (t_sitting.n !== 0 && t_entry2.n !== 0) {
      if (t_sitting.n <= t_entry2.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_entry2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_entry2, j, undefined); } } } }
      else { for (let k = 0; k < t_entry2.n; k++) { const i = t_entry2.live[k]; vgCollectType(t_entry2, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_entry2, i, undefined); } } } }
    }
    // sitting exit1 > stepBack
    if (t_sitting.n !== 0 && t_exit1.n !== 0) {
      if (t_sitting.n <= t_exit1.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_exit1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_exit1, j, undefined); } } } }
      else { for (let k = 0; k < t_exit1.n; k++) { const i = t_exit1.live[k]; vgCollectType(t_exit1, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_exit1, i, undefined); } } } }
    }
    // sitting exit2 > stepBack
    if (t_sitting.n !== 0 && t_exit2.n !== 0) {
      if (t_sitting.n <= t_exit2.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_exit2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_exit2, j, undefined); } } } }
      else { for (let k = 0; k < t_exit2.n; k++) { const i = t_exit2.live[k]; vgCollectType(t_exit2, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_exit2, i, undefined); } } } }
    }
    // sitting wall > stepBack
    if (t_sitting.n !== 0 && t_wall.n !== 0) {
      if (t_sitting.n <= t_wall.n) { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[i]) { VG_EFFECTS.stepBack(t_sitting, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_sitting.killed[j]) { VG_EFFECTS.stepBack(t_sitting, j, t_wall, i, undefined); } } } }
    }
    // avatar wall > stepBack
    ...
    // ---- bounce pass ----
    // vertical wall > reverseDirection
    if (t_vertical.n !== 0 && t_wall.n !== 0) {
      if (t_vertical.n <= t_wall.n) { for (let k = 0; k < t_vertical.n; k++) { const i = t_vertical.live[k]; vgCollectType(t_vertical, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_vertical.killed[i]) { VG_EFFECTS.reverseDirection(t_vertical, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_vertical); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_vertical.killed[j]) { VG_EFFECTS.reverseDirection(t_vertical, j, t_wall, i, undefined); } } } }
    }
    // horizontal wall > reverseDirection
    if (t_horizontal.n !== 0 && t_wall.n !== 0) {
      if (t_horizontal.n <= t_wall.n) { for (let k = 0; k < t_horizontal.n; k++) { const i = t_horizontal.live[k]; vgCollectType(t_horizontal, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_horizontal.killed[i]) { VG_EFFECTS.reverseDirection(t_horizontal, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_horizontal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_horizontal.killed[j]) { VG_EFFECTS.reverseDirection(t_horizontal, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // random entry1 > stepBack
    if (t_random.n !== 0 && t_entry1.n !== 0) {
    ...
    // ---- non-move effects ----
    // avatar entry1 > teleportToExit
    if (t_avatar.n !== 0 && t_entry1.n !== 0) {
      if (t_avatar.n <= t_entry1.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_entry1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG_EFFECTS.teleportToExit(t_avatar, i, t_entry1, j, undefined); } } } }
      else { for (let k = 0; k < t_entry1.n; k++) { const i = t_entry1.live[k]; vgCollectType(t_entry1, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG_EFFECTS.teleportToExit(t_avatar, j, t_entry1, i, undefined); } } } }
    }
    // avatar entry2 > teleportToExit
    if (t_avatar.n !== 0 && t_entry2.n !== 0) {
      if (t_avatar.n <= t_entry2.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_entry2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG_EFFECTS.teleportToExit(t_avatar, i, t_entry2, j, undefined); } } } }
      else { for (let k = 0; k < t_entry2.n; k++) { const i = t_entry2.live[k]; vgCollectType(t_entry2, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG_EFFECTS.teleportToExit(t_avatar, j, t_entry2, i, undefined); } } } }
    }
    // goal avatar > killSprite scoreChange=1
    ...
    // avatar sitting > killSprite
    if (t_avatar.n !== 0 && t_sitting.n !== 0) {
      if (t_avatar.n <= t_sitting.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_sitting); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_sitting.n; k++) { const i = t_sitting.live[k]; vgCollectType(t_sitting, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar vertical > killSprite
    if (t_avatar.n !== 0 && t_vertical.n !== 0) {
      if (t_avatar.n <= t_vertical.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_vertical); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_vertical.n; k++) { const i = t_vertical.live[k]; vgCollectType(t_vertical, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar horizontal > killSprite
    if (t_avatar.n !== 0 && t_horizontal.n !== 0) {
      if (t_avatar.n <= t_horizontal.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_horizontal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_horizontal.n; k++) { const i = t_horizontal.live[k]; vgCollectType(t_horizontal, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // (static type: never leaves the screen)
    // horizontal EOS > killSprite
    for (let k = t_horizontal.n - 1; k >= 0; k--) { const i = t_horizontal.live[k]; if (vgContains(t_horizontal, i)) continue; vgKill(t_horizontal, i); if (!vgContains(t_horizontal, i)) vgKill(t_horizontal, i); }
    // random EOS > killSprite
    for (let k = t_random.n - 1; k >= 0; k--) { const i = t_random.live[k]; if (vgContains(t_random, i)) continue; vgKill(t_random, i); if (!vgContains(t_random, i)) vgKill(t_random, i); }
    // sitting EOS > killSprite
    // (static type: never leaves the screen)
    // vertical EOS > killSprite
    for (let k = t_vertical.n - 1; k >= 0; k--) { const i = t_vertical.live[k]; if (vgContains(t_vertical, i)) continue; vgKill(t_vertical, i); if (!vgContains(t_vertical, i)) vgKill(t_vertical, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
    ...
    VG.ended = false; VG.won = false;
    if (t_goal.n <= 0) { VG.ended = true; VG.won = true; }   // SpriteCounter stype=goal limit=0 win=true
    else if (t_avatar.n <= 0) { VG.ended = true; VG.won = false; }   // SpriteCounter stype=avatar limit=0 win=false
