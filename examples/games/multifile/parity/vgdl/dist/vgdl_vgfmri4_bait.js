...
// ---- compiled tick: 7 types, 16 rules (tools/compile_vgdl.mjs, profile rcrl) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  const T = VG.types;
  const t_hole = T[0];   // hole > Immovable (inert)
  const t_mushroom = T[1];   // mushroom > Immovable (inert)
  const t_key = T[2];   // key > Resource
  const t_goal = T[3];   // goal > Immovable (inert)
  const t_box = T[4];   // box > Passive
  const t_wall = T[5];   // wall > Immovable (inert)
  const t_avatar = T[6];   // avatar > MovingAvatar
  const EFF = VG.rcEffects;
  function events() {
    const cache = {};
    const dead = new Set();
    ...
    while (again) {
      const nc = new Set();
      let g_hole = null, g_box = null, g_avatar = null, g_key = null, g_mushroom = null, g_goal = null, g_wall = null;   // group snapshots, one lookup per stype per pass
      // hole box > changeScore value=1
      { const e1 = g_hole || (g_hole = rcGroupEntry("hole", cache)), l1 = e1.list;
        const e2 = g_box || (g_box = rcGroupEntry("box", cache));
      ...
      // mushroom avatar > changeScore value=1
      { const e1 = g_mushroom || (g_mushroom = rcGroupEntry("mushroom", cache)), l1 = e1.list;
      ...
      // avatar hole > killSprite
      { const e1 = g_avatar || (g_avatar = rcGroupEntry("avatar", cache)), l1 = e1.list;
        const e2 = g_hole || (g_hole = rcGroupEntry("hole", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
      ...
      { const e1 = g_box || (g_box = rcGroupEntry("box", cache)), l1 = e1.list;
        ...
        g_hole = null; g_box = null; g_avatar = null; g_key = null; g_mushroom = null; g_goal = null; g_wall = null;   // _updateCollisionDict may have invalidated snapshots
      }
      // box wall > stepBack
      { const e1 = g_box || (g_box = rcGroupEntry("box", cache)), l1 = e1.list;
        const e2 = g_wall || (g_wall = rcGroupEntry("wall", cache));
      ...
      // box mushroom > undoAll
      { const e1 = g_box || (g_box = rcGroupEntry("box", cache)), l1 = e1.list;
        const e2 = g_mushroom || (g_mushroom = rcGroupEntry("mushroom", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
          for (let u = 0; u < l1.length; u += 2) { const s1 = l1[u], i1 = l1[u + 1]; rcCollect(s1, i1, e2); if (rcCand.length === 0) continue; const cand = rcCand.slice();
  ...
  function tick(keys) {
    ...
    for (let k = 0; k < t_box.n; k++) { const i = t_box.live[k]; if (!t_box.killed[i]) rcUpdBase(t_box, i); }   // box > Passive
    ...
    if (rcLiveCount(t_goal) <= 0) { if (VG.time > VG.spriteBonusT) { VG.score += 5; VG.spriteBonusT = VG.time; } VG.ended = true; VG.won = true; return; }   // SpriteCounter stype=goal limit=0 win=true bonus=5
  }
  VG.tickImpl = tick;
};
