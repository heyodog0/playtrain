// vgdl/infer/pushBoulders — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 8 types, 30 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_box1 = T[2];   // box1 > Passive
  const t_box2 = T[3];   // box2 > Immovable (inert: never updated, never leaves the screen)
  const t_goal = T[4];   // goal > Immovable (inert: never updated, never leaves the screen)
  const t_poison1 = T[5];   // poison1 > Immovable (inert: never updated, never leaves the screen)
  const t_poison2 = T[6];   // poison2 > Passive
  const t_wall = T[7];   // wall > Immovable (inert: never updated, never leaves the screen)
  ...
  function tick(keys) {
    ...
    if (t_box1.jpDirty) { for (let k = 0; k < t_box1.n; k++) t_box1.jpT[t_box1.live[k]] = -1; t_box1.jpDirty = false; }
    if (t_poison2.jpDirty) { for (let k = 0; k < t_poison2.n; k++) t_poison2.jpT[t_poison2.live[k]] = -1; t_poison2.jpDirty = false; }
    const n0_avatar = t_avatar.n;
    const n0_box1 = t_box1.n;
    const n0_poison2 = t_poison2.n;
    for (let k = 0; k < n0_avatar; k++) vgUpdMoving(t_avatar, t_avatar.live[k]);   // avatar > MovingAvatar
    for (let k = 0; k < n0_box1; k++) vgBaseUpdate(t_box1, t_box1.live[k]);   // box1 > Passive
    for (let k = 0; k < n0_poison2; k++) vgBaseUpdate(t_poison2, t_poison2.live[k]);   // poison2 > Passive
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // box1 box1 > stepBack
    if (t_box1.n > 1) for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_box1); const nc = vgCand.length;
      for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (j === i) continue; if (!t_box1.killed[i]) { VG_EFFECTS.stepBack(t_box1, i, t_box1, j, undefined); } } }
    // box1 wall > stepBack
    if (t_box1.n !== 0 && t_wall.n !== 0) {
      if (t_box1.n <= t_wall.n) { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box1.killed[i]) { VG_EFFECTS.stepBack(t_box1, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box1.killed[j]) { VG_EFFECTS.stepBack(t_box1, j, t_wall, i, undefined); } } } }
    }
    // box2 goal > stepBack
    if (t_box2.n !== 0 && t_goal.n !== 0) {
      if (t_box2.n <= t_goal.n) { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[i]) { VG_EFFECTS.stepBack(t_box2, i, t_goal, j, undefined); } } } }
      else { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[j]) { VG_EFFECTS.stepBack(t_box2, j, t_goal, i, undefined); } } } }
    }
    // box2 wall > stepBack
    if (t_box2.n !== 0 && t_wall.n !== 0) {
      if (t_box2.n <= t_wall.n) { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[i]) { VG_EFFECTS.stepBack(t_box2, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[j]) { VG_EFFECTS.stepBack(t_box2, j, t_wall, i, undefined); } } } }
    }
    // goal poison1 > stepBack
    if (t_goal.n !== 0 && t_poison1.n !== 0) {
      if (t_goal.n <= t_poison1.n) { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_poison1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[i]) { VG_EFFECTS.stepBack(t_goal, i, t_poison1, j, undefined); } } } }
      else { for (let k = 0; k < t_poison1.n; k++) { const i = t_poison1.live[k]; vgCollectType(t_poison1, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[j]) { VG_EFFECTS.stepBack(t_goal, j, t_poison1, i, undefined); } } } }
    }
    // goal poison2 > stepBack
    if (t_goal.n !== 0 && t_poison2.n !== 0) {
      if (t_goal.n <= t_poison2.n) { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[i]) { VG_EFFECTS.stepBack(t_goal, i, t_poison2, j, undefined); } } } }
      else { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[j]) { VG_EFFECTS.stepBack(t_goal, j, t_poison2, i, undefined); } } } }
    }
    // goal wall > stepBack
    if (t_goal.n !== 0 && t_wall.n !== 0) {
      if (t_goal.n <= t_wall.n) { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[i]) { VG_EFFECTS.stepBack(t_goal, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[j]) { VG_EFFECTS.stepBack(t_goal, j, t_wall, i, undefined); } } } }
    }
    // poison1 wall > stepBack
    if (t_poison1.n !== 0 && t_wall.n !== 0) {
      if (t_poison1.n <= t_wall.n) { for (let k = 0; k < t_poison1.n; k++) { const i = t_poison1.live[k]; vgCollectType(t_poison1, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[i]) { VG_EFFECTS.stepBack(t_poison1, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_poison1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[j]) { VG_EFFECTS.stepBack(t_poison1, j, t_wall, i, undefined); } } } }
    }
    // poison2 wall > stepBack
    if (t_poison2.n !== 0 && t_wall.n !== 0) {
      if (t_poison2.n <= t_wall.n) { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[i]) { VG_EFFECTS.stepBack(t_poison2, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[j]) { VG_EFFECTS.stepBack(t_poison2, j, t_wall, i, undefined); } } } }
    }
    // poison1 box2 > stepBack
    if (t_poison1.n !== 0 && t_box2.n !== 0) {
      if (t_poison1.n <= t_box2.n) { for (let k = 0; k < t_poison1.n; k++) { const i = t_poison1.live[k]; vgCollectType(t_poison1, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[i]) { VG_EFFECTS.stepBack(t_poison1, i, t_box2, j, undefined); } } } }
      else { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_poison1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[j]) { VG_EFFECTS.stepBack(t_poison1, j, t_box2, i, undefined); } } } }
    }
    // poison2 box1 > stepBack
    if (t_poison2.n !== 0 && t_box1.n !== 0) {
      if (t_poison2.n <= t_box1.n) { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[i]) { VG_EFFECTS.stepBack(t_poison2, i, t_box1, j, undefined); } } } }
      else { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[j]) { VG_EFFECTS.stepBack(t_poison2, j, t_box1, i, undefined); } } } }
    }
    // poison2 box2 > stepBack
    if (t_poison2.n !== 0 && t_box2.n !== 0) {
      if (t_poison2.n <= t_box2.n) { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[i]) { VG_EFFECTS.stepBack(t_poison2, i, t_box2, j, undefined); } } } }
      else { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[j]) { VG_EFFECTS.stepBack(t_poison2, j, t_box2, i, undefined); } } } }
    }
    // poison2 poison2 > stepBack
    if (t_poison2.n > 1) for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_poison2); const nc = vgCand.length;
      for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (j === i) continue; if (!t_poison2.killed[i]) { VG_EFFECTS.stepBack(t_poison2, i, t_poison2, j, undefined); } } }
    // ---- bounce pass ----
    // box1 avatar > bounceForward
    if (t_box1.n !== 0 && t_avatar.n !== 0) {
      if (t_box1.n <= t_avatar.n) { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box1.killed[i]) { VG_EFFECTS.bounceForward(t_box1, i, t_avatar, j, undefined); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box1.killed[j]) { VG_EFFECTS.bounceForward(t_box1, j, t_avatar, i, undefined); } } } }
    }
    // poison2 box1 > bounceForward
    if (t_poison2.n !== 0 && t_box1.n !== 0) {
      if (t_poison2.n <= t_box1.n) { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[i]) { VG_EFFECTS.bounceForward(t_poison2, i, t_box1, j, undefined); } } } }
      else { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison2.killed[j]) { VG_EFFECTS.bounceForward(t_poison2, j, t_box1, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // avatar wall > stepBack
    if (t_avatar.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // avatar poison1 > killSprite
    if (t_avatar.n !== 0 && t_poison1.n !== 0) {
      if (t_avatar.n <= t_poison1.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_poison1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_poison1.n; k++) { const i = t_poison1.live[k]; vgCollectType(t_poison1, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // avatar poison2 > killSprite
    if (t_avatar.n !== 0 && t_poison2.n !== 0) {
      if (t_avatar.n <= t_poison2.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_poison2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_poison2.n; k++) { const i = t_poison2.live[k]; vgCollectType(t_poison2, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { vgKill(t_avatar, j); } } } }
    }
    // box2 avatar > killSprite
    if (t_box2.n !== 0 && t_avatar.n !== 0) {
      if (t_box2.n <= t_avatar.n) { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[i]) { vgKill(t_box2, i); } } } }
      else { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[j]) { vgKill(t_box2, j); } } } }
    }
    // box2 box1 > killSprite
    if (t_box2.n !== 0 && t_box1.n !== 0) {
      if (t_box2.n <= t_box1.n) { for (let k = 0; k < t_box2.n; k++) { const i = t_box2.live[k]; vgCollectType(t_box2, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[i]) { vgKill(t_box2, i); } } } }
      else { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_box2); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_box2.killed[j]) { vgKill(t_box2, j); } } } }
    }
    // goal avatar > killSprite scoreChange=1
    ...
    // goal box1 > killSprite scoreChange=1
    if (t_goal.n !== 0 && t_box1.n !== 0) {
      if (t_goal.n <= t_box1.n) { for (let k = 0; k < t_goal.n; k++) { const i = t_goal.live[k]; vgCollectType(t_goal, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[i]) { VG.score += 1; vgKill(t_goal, i); } } } }
      else { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_goal); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_goal.killed[j]) { VG.score += 1; vgKill(t_goal, j); } } } }
    }
    // poison1 box1 > killSprite
    if (t_poison1.n !== 0 && t_box1.n !== 0) {
      if (t_poison1.n <= t_box1.n) { for (let k = 0; k < t_poison1.n; k++) { const i = t_poison1.live[k]; vgCollectType(t_poison1, i, t_box1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[i]) { vgKill(t_poison1, i); } } } }
      else { for (let k = 0; k < t_box1.n; k++) { const i = t_box1.live[k]; vgCollectType(t_box1, i, t_poison1); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_poison1.killed[j]) { vgKill(t_poison1, j); } } } }
    }
    // avatar EOS > killSprite
    ...
    // box1 EOS > killSprite
    for (let k = t_box1.n - 1; k >= 0; k--) { const i = t_box1.live[k]; if (vgContains(t_box1, i)) continue; vgKill(t_box1, i); if (!vgContains(t_box1, i)) vgKill(t_box1, i); }
    // box2 EOS > killSprite
    ...
    // (static type: never leaves the screen)
    // poison2 EOS > killSprite
    for (let k = t_poison2.n - 1; k >= 0; k--) { const i = t_poison2.live[k]; if (vgContains(t_poison2, i)) continue; vgKill(t_poison2, i); if (!vgContains(t_poison2, i)) vgKill(t_poison2, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
