// vgdl/infer/avoidGeorge — GENERATED by tools/bundle_vgdl.mjs, do not edit.
...
// ---- compiled tick: 9 types, 17 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_annoyed = T[1];   // annoyed > RandomNPC
  const t_cigarette = T[2];   // cigarette > Flicker
  const t_quiet = T[3];   // quiet > RandomNPC
  const t_avatar = T[4];   // avatar > ShootAvatar
  const t_george = T[5];   // george > Chaser
  const t_kennel = T[6];   // kennel > Immovable (inert: never updated, never leaves the screen)
  const t_puppy = T[7];   // puppy > Chaser
  const t_wall = T[8];   // wall > Immovable (inert: never updated, never leaves the screen)
  const ARGS_0 = {"stype":"quiet"};
  const ARGS_1 = {"stype":"quiet"};
  const ARGS_5 = {"stype":"puppy"};
  const ARGS_6 = {"stype":"annoyed"};
  ...
  function tick(keys) {
    ...
    if (t_annoyed.jpDirty) { for (let k = 0; k < t_annoyed.n; k++) t_annoyed.jpT[t_annoyed.live[k]] = -1; t_annoyed.jpDirty = false; }
    if (t_cigarette.jpDirty) { for (let k = 0; k < t_cigarette.n; k++) t_cigarette.jpT[t_cigarette.live[k]] = -1; t_cigarette.jpDirty = false; }
    if (t_quiet.jpDirty) { for (let k = 0; k < t_quiet.n; k++) t_quiet.jpT[t_quiet.live[k]] = -1; t_quiet.jpDirty = false; }
    if (t_avatar.jpDirty) { for (let k = 0; k < t_avatar.n; k++) t_avatar.jpT[t_avatar.live[k]] = -1; t_avatar.jpDirty = false; }
    if (t_george.jpDirty) { for (let k = 0; k < t_george.n; k++) t_george.jpT[t_george.live[k]] = -1; t_george.jpDirty = false; }
    if (t_puppy.jpDirty) { for (let k = 0; k < t_puppy.n; k++) t_puppy.jpT[t_puppy.live[k]] = -1; t_puppy.jpDirty = false; }
    const n0_annoyed = t_annoyed.n;
    const n0_cigarette = t_cigarette.n;
    const n0_quiet = t_quiet.n;
    const n0_avatar = t_avatar.n;
    const n0_george = t_george.n;
    const n0_puppy = t_puppy.n;
    for (let k = 0; k < n0_annoyed; k++) vgUpdRandom(t_annoyed, t_annoyed.live[k]);   // annoyed > RandomNPC
    for (let k = 0; k < n0_cigarette; k++) vgUpdFlicker(t_cigarette, t_cigarette.live[k]);   // cigarette > Flicker
    for (let k = 0; k < n0_quiet; k++) vgUpdRandom(t_quiet, t_quiet.live[k]);   // quiet > RandomNPC
    for (let k = 0; k < n0_avatar; k++) vgUpdShoot(t_avatar, t_avatar.live[k]);   // avatar > ShootAvatar
    for (let k = 0; k < n0_george; k++) vgUpdChaser(t_george, t_george.live[k]);   // george > Chaser
    for (let k = 0; k < n0_puppy; k++) vgUpdChaser(t_puppy, t_puppy.live[k]);   // puppy > Chaser
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // annoyed wall > stepBack
    if (t_annoyed.n !== 0 && t_wall.n !== 0) {
      if (t_annoyed.n <= t_wall.n) { for (let k = 0; k < t_annoyed.n; k++) { const i = t_annoyed.live[k]; vgCollectType(t_annoyed, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[i]) { VG_EFFECTS.stepBack(t_annoyed, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_annoyed); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[j]) { VG_EFFECTS.stepBack(t_annoyed, j, t_wall, i, undefined); } } } }
    }
    // avatar wall > stepBack
    ...
    // quiet wall > stepBack
    if (t_quiet.n !== 0 && t_wall.n !== 0) {
      if (t_quiet.n <= t_wall.n) { for (let k = 0; k < t_quiet.n; k++) { const i = t_quiet.live[k]; vgCollectType(t_quiet, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_quiet.killed[i]) { VG_EFFECTS.stepBack(t_quiet, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_quiet); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_quiet.killed[j]) { VG_EFFECTS.stepBack(t_quiet, j, t_wall, i, undefined); } } } }
    }
    // puppy wall > stepBack
    if (t_puppy.n !== 0 && t_wall.n !== 0) {
      if (t_puppy.n <= t_wall.n) { for (let k = 0; k < t_puppy.n; k++) { const i = t_puppy.live[k]; vgCollectType(t_puppy, i, t_wall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_puppy.killed[i]) { VG_EFFECTS.stepBack(t_puppy, i, t_wall, j, undefined); } } } }
      else { for (let k = 0; k < t_wall.n; k++) { const i = t_wall.live[k]; vgCollectType(t_wall, i, t_puppy); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_puppy.killed[j]) { VG_EFFECTS.stepBack(t_puppy, j, t_wall, i, undefined); } } } }
    }
    // ---- stepBack pass 2 ----
    // annoyed wall > stepBack
    if (t_annoyed.n !== 0 && t_wall.n !== 0) {
    ...
    // ---- non-move effects ----
    // annoyed cigarette > transformTo stype=quiet scoreChange=1
    if (t_annoyed.n !== 0 && t_cigarette.n !== 0) {
      if (t_annoyed.n <= t_cigarette.n) { for (let k = 0; k < t_annoyed.n; k++) { const i = t_annoyed.live[k]; vgCollectType(t_annoyed, i, t_cigarette); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[i]) { VG.score += 1; VG_EFFECTS.transformTo(t_annoyed, i, t_cigarette, j, ARGS_0); } } } }
      else { for (let k = 0; k < t_cigarette.n; k++) { const i = t_cigarette.live[k]; vgCollectType(t_cigarette, i, t_annoyed); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[j]) { VG.score += 1; VG_EFFECTS.transformTo(t_annoyed, j, t_cigarette, i, ARGS_0); } } } }
    }
    // annoyed puppy > transformTo stype=quiet scoreChange=1
    if (t_annoyed.n !== 0 && t_puppy.n !== 0) {
      if (t_annoyed.n <= t_puppy.n) { for (let k = 0; k < t_annoyed.n; k++) { const i = t_annoyed.live[k]; vgCollectType(t_annoyed, i, t_puppy); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[i]) { VG.score += 1; VG_EFFECTS.transformTo(t_annoyed, i, t_puppy, j, ARGS_1); } } } }
      else { for (let k = 0; k < t_puppy.n; k++) { const i = t_puppy.live[k]; vgCollectType(t_puppy, i, t_annoyed); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_annoyed.killed[j]) { VG.score += 1; VG_EFFECTS.transformTo(t_annoyed, j, t_puppy, i, ARGS_1); } } } }
    }
    // avatar george > killSprite scoreChange=-1
    if (t_avatar.n !== 0 && t_george.n !== 0) {
      if (t_avatar.n <= t_george.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_george); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG.score += -1; vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_george.n; k++) { const i = t_george.live[k]; vgCollectType(t_george, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG.score += -1; vgKill(t_avatar, j); } } } }
    }
    // kennel cigarette > transformTo stype=puppy
    if (t_kennel.n !== 0 && t_cigarette.n !== 0) {
      if (t_kennel.n <= t_cigarette.n) { for (let k = 0; k < t_kennel.n; k++) { const i = t_kennel.live[k]; vgCollectType(t_kennel, i, t_cigarette); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_kennel.killed[i]) { VG_EFFECTS.transformTo(t_kennel, i, t_cigarette, j, ARGS_5); } } } }
      else { for (let k = 0; k < t_cigarette.n; k++) { const i = t_cigarette.live[k]; vgCollectType(t_cigarette, i, t_kennel); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_kennel.killed[j]) { VG_EFFECTS.transformTo(t_kennel, j, t_cigarette, i, ARGS_5); } } } }
    }
    // quiet george > transformTo stype=annoyed
    if (t_quiet.n !== 0 && t_george.n !== 0) {
      if (t_quiet.n <= t_george.n) { for (let k = 0; k < t_quiet.n; k++) { const i = t_quiet.live[k]; vgCollectType(t_quiet, i, t_george); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_quiet.killed[i]) { VG_EFFECTS.transformTo(t_quiet, i, t_george, j, ARGS_6); } } } }
      else { for (let k = 0; k < t_george.n; k++) { const i = t_george.live[k]; vgCollectType(t_george, i, t_quiet); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_quiet.killed[j]) { VG_EFFECTS.transformTo(t_quiet, j, t_george, i, ARGS_6); } } } }
    }
    // annoyed EOS > killSprite
    for (let k = t_annoyed.n - 1; k >= 0; k--) { const i = t_annoyed.live[k]; if (vgContains(t_annoyed, i)) continue; vgKill(t_annoyed, i); if (!vgContains(t_annoyed, i)) vgKill(t_annoyed, i); }
    // avatar EOS > killSprite
    ...
    // cigarette EOS > killSprite
    for (let k = t_cigarette.n - 1; k >= 0; k--) { const i = t_cigarette.live[k]; if (vgContains(t_cigarette, i)) continue; vgKill(t_cigarette, i); if (!vgContains(t_cigarette, i)) vgKill(t_cigarette, i); }
    // george EOS > killSprite
    for (let k = t_george.n - 1; k >= 0; k--) { const i = t_george.live[k]; if (vgContains(t_george, i)) continue; vgKill(t_george, i); if (!vgContains(t_george, i)) vgKill(t_george, i); }
    // kennel EOS > killSprite
    // (static type: never leaves the screen)
    // puppy EOS > killSprite
    for (let k = t_puppy.n - 1; k >= 0; k--) { const i = t_puppy.live[k]; if (vgContains(t_puppy, i)) continue; vgKill(t_puppy, i); if (!vgContains(t_puppy, i)) vgKill(t_puppy, i); }
    // quiet EOS > killSprite
    for (let k = t_quiet.n - 1; k >= 0; k--) { const i = t_quiet.live[k]; if (vgContains(t_quiet, i)) continue; vgKill(t_quiet, i); if (!vgContains(t_quiet, i)) vgKill(t_quiet, i); }
    // wall EOS > killSprite
    // (static type: never leaves the screen)
    VG.ss = null;
    vgFlush();
    ...
    VG.ended = false; VG.won = false;
    if (t_avatar.n <= 0) { VG.ended = true; VG.won = false; }   // SpriteCounter stype=avatar win=false
    else if (t_quiet.n <= 0) { VG.ended = true; VG.won = false; }   // SpriteCounter stype=quiet win=false
    else if (VG.time >= 500) { VG.ended = true; VG.won = true; }   // Timeout limit=500 win=true
  }
  VG.tickImpl = tick;
};
