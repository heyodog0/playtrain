// vgdl/infer/picoparkish — GENERATED by tools/bundle_vgdl.mjs, do not edit.
// reference: py-vgdl (see manifest.json); block=50 render=tiles levels=4
const VG_GAME_TEXT = "BasicGame\n    SpriteSet\n        floor > Immovable img=colors/LIGHTGRAY\n        wall > Immovable img=colors/DARKGRAY\n        breakwall > Immovable img=colors/BROWN\n        button > Immovable img=colors/YELLOW\n        exit > Immovable img=colors/GREEN\n        avatar > GravityAvatar img=colors/RED\n\n    LevelMapping\n        . > floor\n        # > floor wall\n        W > floor breakwall\n        B > floor button\n        D > floor exit\n        A > floor avatar\n\n    TerminationSet\n        SpriteCounter stype=avatar limit=0 win=False\n\n    InteractionSet\n        avatar wall > stepBack\n        avatar breakwall > stepBack\n        \n        avatar button > DestroyAllBreakwalls\n        avatar exit > killSprite scoreChange=100\n        \n        avatar EOS > stepBack\n        wall EOS > killSprite\n        breakwall EOS > killSprite\n        button EOS > killSprite\n        exit EOS > killSprite";
const VG_LEVELS = ["############\n#..........#\n#.....W...D#\n#.....W....#\n#A...WW.WB.#\n############","################\n#..............#\n#...WWWWWW.....#\n#...WWWWWW.....#\n#...WW..BW.....#\n#A.....WWW...D.#\n################","####################\n#1.................#\n#..................#\n#2.....WWWWWWW.....#\n#......WWWWWWW.....#\n#......WWWWWWW.....#\n#..................#\n#.........B........#\n#..................#\n#.........D........#\n####################","############\n#1.........#\n#2.........#\n#.W.W.W.W..#\n#.W.W.W.W..#\n#.W.B.W.W.D#\n############"];
const VG_BLOCK = 50;
...
// ---- compiled tick: 6 types, 9 rules (tools/compile_vgdl.mjs, profile colas) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  ...
  const t_breakwall = T[2];   // breakwall > Immovable (inert: never updated, never leaves the screen)
  const t_button = T[3];   // button > Immovable (inert: never updated, never leaves the screen)
  const t_exit = T[4];   // exit > Immovable (inert: never updated, never leaves the screen)
  const t_avatar = T[5];   // avatar > GravityAvatar
  ...
  function tick(keys) {
    ...
    for (let k = 0; k < n0_avatar; k++) vgUpdGravity(t_avatar, t_avatar.live[k]);   // avatar > GravityAvatar
    VG.ss = {};
    // ---- stepBack pass 1 ----
    // avatar wall > stepBack
    ...
    // avatar breakwall > stepBack
    if (t_avatar.n !== 0 && t_breakwall.n !== 0) {
      if (t_avatar.n <= t_breakwall.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_breakwall); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG_EFFECTS.stepBack(t_avatar, i, t_breakwall, j, undefined); } } } }
      else { for (let k = 0; k < t_breakwall.n; k++) { const i = t_breakwall.live[k]; vgCollectType(t_breakwall, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG_EFFECTS.stepBack(t_avatar, j, t_breakwall, i, undefined); } } } }
    }
    // avatar EOS > stepBack
    ...
    // ---- non-move effects ----
    // avatar button > DestroyAllBreakwalls
    if (t_avatar.n !== 0 && t_button.n !== 0) {
      if (t_avatar.n <= t_button.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_button); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG_EFFECTS.DestroyAllBreakwalls(t_avatar, i, t_button, j, undefined); } } } }
      else { for (let k = 0; k < t_button.n; k++) { const i = t_button.live[k]; vgCollectType(t_button, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG_EFFECTS.DestroyAllBreakwalls(t_avatar, j, t_button, i, undefined); } } } }
    }
    // avatar exit > killSprite scoreChange=100
    if (t_avatar.n !== 0 && t_exit.n !== 0) {
      if (t_avatar.n <= t_exit.n) { for (let k = 0; k < t_avatar.n; k++) { const i = t_avatar.live[k]; vgCollectType(t_avatar, i, t_exit); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[i]) { VG.score += 100; vgKill(t_avatar, i); } } } }
      else { for (let k = 0; k < t_exit.n; k++) { const i = t_exit.live[k]; vgCollectType(t_exit, i, t_avatar); const nc = vgCand.length;
        for (let q = 0; q < nc; q++) { const j = vgCand[q]; if (!t_avatar.killed[j]) { VG.score += 100; vgKill(t_avatar, j); } } } }
    }
    // wall EOS > killSprite
