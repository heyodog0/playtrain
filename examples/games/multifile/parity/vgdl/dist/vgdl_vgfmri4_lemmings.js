...
const VG_GROUP_ORDER = ["entrance", "shovel", "goal", "wall", "lemming", "avatar", "hole"];
...
// ---- compiled tick: 7 types, 12 rules (tools/compile_vgdl.mjs, profile rcrl) ----
VG.installCompiled = function () {   // called by vgPrepare once the type table is resolved
  const T = VG.types;
  const t_hole = T[0];   // hole > Immovable (inert)
  const t_shovel = T[1];   // shovel > Flicker
  const t_entrance = T[2];   // entrance > SpawnPoint
  const t_goal = T[3];   // goal > Immovable (inert)
  const t_lemming = T[4];   // lemming > Chaser
  const t_wall = T[5];   // wall > Immovable (inert)
  const t_avatar = T[6];   // avatar > ShootAvatar
  const EFF = VG.rcEffects;
  function events() {
    const cache = {};
    const dead = new Set();
    ...
    while (again) {
      const nc = new Set();
      let g_lemming = null, g_goal = null, g_avatar = null, g_hole = null, g_wall = null, g_shovel = null;   // group snapshots, one lookup per stype per pass
      // lemming goal > changeScore value=2
      { const e1 = g_lemming || (g_lemming = rcGroupEntry("lemming", cache)), l1 = e1.list;
      ...
      // wall shovel > changeScore value=-1
      { const e1 = g_wall || (g_wall = rcGroupEntry("wall", cache)), l1 = e1.list;
        const e2 = g_shovel || (g_shovel = rcGroupEntry("shovel", cache));
        if (l1.length !== 0 && e2.list.length !== 0) {
      ...
      // avatar EOS > stepBack
      { const e1 = g_avatar || (g_avatar = rcGroupEntry("avatar", cache)), l1 = e1.list;
        for (let u = 0; u < l1.length; u += 2) { const s = l1[u], si = l1[u + 1]; if (!vgContains(s, si)) EFF[10].fn(s, si, null, -1, EFF[10].args); } }
      // lemming EOS > stepBack
      { const e1 = g_lemming || (g_lemming = rcGroupEntry("lemming", cache)), l1 = e1.list;
        for (let u = 0; u < l1.length; u += 2) { const s = l1[u], si = l1[u + 1]; if (!vgContains(s, si)) EFF[11].fn(s, si, null, -1, EFF[11].args); } }
  ...
  function tick(keys) {
    ...
    const noop = keys !== null && keys.length === 0;
    VG.keys = keys || [];
    for (let k = 0; k < t_shovel.n; k++) { const i = t_shovel.live[k]; if (!t_shovel.killed[i]) rcUpdFlicker(t_shovel, i); }   // shovel > Flicker
    for (let k = 0; k < t_entrance.n; k++) { const i = t_entrance.live[k]; if (!t_entrance.killed[i]) rcUpdSpawn(t_entrance, i); }   // entrance > SpawnPoint
    for (let k = 0; k < t_lemming.n; k++) { const i = t_lemming.live[k]; if (!t_lemming.killed[i]) rcUpdChaser(t_lemming, i); }   // lemming > Chaser
    ...
    if ((rcLiveCount(t_entrance) + rcLiveCount(t_lemming)) === 0) { if (VG.time > VG.spriteBonusT) { VG.score += 5; VG.spriteBonusT = VG.time; } VG.ended = true; VG.won = true; return; }   // MultiSpriteCounter stype1=entrance stype2=lemming limit=0 win=true bonus=5
  }
  VG.tickImpl = tick;
};
