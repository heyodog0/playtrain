...
// ---- ../craftax_fp/src/80_render_fp.js ----
...
// ---- src/90_playtrain_free.js ----
// 90_playtrain_free.js — the PlayTrain contract for the free-movement variant.
//
// craftax_fp's 90_playtrain_fp.js with a different KEYMAP: the six actions
// 72_move_free.js adds are what the arrows drive here, so up/down walk forward
// and back without turning and left/right turn in place. The
...
function currentAction() {
  ...
  // Presses first, so a single keystroke cannot craft twice while an arrow can
  // be held to walk — craftax_fp's rule, unchanged.
  ...
  // The point of this variant. These need no facing-relative remap, because
  // the actions themselves are relative — which is why, unlike craftax_fp,
  // this game does NOT define relativeArrow(): the play page has nothing to
  // translate, and an agent sending action 17 gets "forward" too.
  if (keyIsDown(38)) return ACT_MOVE_FORWARD;   // up
  if (keyIsDown(40)) return ACT_MOVE_BACK;      // down
  if (keyIsDown(37)) return ACT_TURN_LEFT;      // left
  if (keyIsDown(39)) return ACT_TURN_RIGHT;     // right
  if (keyIsDown(65)) return ACT_STRAFE_LEFT;    // A
  if (keyIsDown(68)) return ACT_STRAFE_RIGHT;   // D

  // Craftax's own absolute moves, kept bound so every one of the original
  // seventeen actions is still reachable from a key. A host drives an action
  // index by pressing its keys, so an unbound action would silently become a
  // NOOP and the "actions 0..16 are unchanged" gate would be a lie.
  if (keyIsDown(74)) return ACT_LEFT;           // J
  if (keyIsDown(76)) return ACT_RIGHT;          // L
  if (keyIsDown(73)) return ACT_UP;             // I
  if (keyIsDown(75)) return ACT_DOWN;           // K
  return ACT_NOOP;
}
