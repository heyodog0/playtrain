// ===== cqhex (analogen_cqhex): cavequest_hard_explore + death penalty =====
// Derived from analogen_cavequest_hard_explore. Env dynamics IDENTICAL except:
//   - DEATH_PENALTY = 5000 (was 0): dying now costs a fixed -5000 score delta,
//     applied like a CURSE (no Math.max(0) score-wipe, so banked gate rewards are
//     NOT erased). Under abs_one this clamps to -1 — a clean "dying is bad" signal
//     to discourage the free-death / respawn churn of the plain explore variant.
//   - STEP_PENALTY stays 0 (no per-frame time penalty).
...
const DEATH_PENALTY = 5000;  // cqhex: fixed death cost, applied like a curse in die()
                             // (no score-wipe floor). Under abs_one clamps to -1;
                             // discourages the free-death churn of the explore variant.
