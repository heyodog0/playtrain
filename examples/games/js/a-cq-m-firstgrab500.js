// a-cq-m-firstgrab500
// Copy of analogen_cavequest_medium with ONE change: a +500 pickup reward is
// RESTORED, but paid ONLY on the FIRST grab of each token in an episode.
// Re-grabbing a dropped item (via persistentDrops) or any later grab of the
// same visualId pays nothing, so there is no drop/re-pickup farm — only the
// initial collection of each distinct item earns. PICKUP_COOLDOWN stays at 14
// (= 2 agent decisions at frame_skip 7), same as medium. Everything else
// (binding/placement map, obstacles, win reward, inv cap) is identical.
//
// --- base header: analogen_cavequest_medium ---
// The "medium" binding-generalization env: analogen_cavequest_easy (6x6) + THREE
// obstacles across THREE rooms. Sits between easy (1 binding, no hazard) and
// analogen_cavequest_hard (8x8, ~5 obstacles).
//
// Three rooms, forced LINEAR path (single passage between consecutive rooms):
//   ROOM1 items + spawn  cols 0-1, rows 0-5     --BLUE DOOR (2,4)-->
//   ROOM2                cols 3-5, rows 4-5      --LASER (4,3)-->
//   ROOM3 goal           cols 3-5, rows 0-2   (goal at (5,0))
//
// Three obstacles (NONE sits on the goal cell):
//   (1) STATIONARY enemy in the item room (tile 3 @ (1,1)) — a hazard among the
//       items. Dodge it, or kill it with a SWORD (consumed, +1000).
//   (2) PATROLLING enemy in the goal room (tile 13 @ (3,1), patrols row 1 of
//       ROOM3 LEFT<->RIGHT) — dodge or kill with a SWORD. Never occupies the goal.
//   (3) LASER gate (tile 10 @ (4,3)) on the ONLY ROOM2->ROOM3 passage. Always
//       active: lethal without BOOTS held, safe with BOOTS. Render/mechanic
//       ported from analogen_cavequest_hard.
//
// BLUE_KEY (door) and BOOTS (laser) are REQUIRED; SWORD is optional insurance vs
// the enemies; HAT/ARMOR stay distractors. Inventory cap is 2 (vs easy's 1) so
// KEY+BOOTS fit together: grab both -> open door (key consumed) -> cross laser
// (boots held) -> reach goal.
//
// Held-out rule UNCHANGED (id12 sword-shape -> BLUE_KEY); bindings stay tool-only
// (360 distinct / 60 held-out / 300 train). Everything else matches easy:
// deliberate standalone-SPACE pickup, NO pickup reward, cd14 pickup cooldown,
// abs_one-friendly step-decay win, item rendering.
//
// ----- analogen_cavequest_easy header -----
//
// ----- v1.9 header -----
...
const TILE_SIZE = 32;
const ROWS = 6;  // medium: 6x6 (same as easy), 3 rooms + 3 obstacles.
const COLS = 6;
...
const MAX_INVENTORY = 2;  // medium: hold two items at once (KEY+BOOTS).
...
let persistentDrops = {};
// a-cq-m-firstgrab500: visualIds already rewarded this episode. Gates the +500
// to the FIRST grab of each token so re-grabs / re-pickups pay nothing.
let pickedUpTools = new Set();
...
function initRoom() {
    // Tiles: 0=floor, 1=wall, 2=key-door, 10=laser, 11=goal, 3=stationary enemy,
    // 13=patrol enemy. (4=sunbeam / 5=blue-door unused here.) 6x6, no outer border.
    //
    // THREE rooms with one passage between consecutive rooms. Forced linear path
    // ROOM1 -> ROOM2 -> ROOM3:
    //   ROOM1 items+spawn  cols 0-1, rows 0-5   (spawn (0,5))
    //   ROOM2              cols 3-5, rows 4-5    (entered via BLUE DOOR at (2,4))
    //   ROOM3 goal         cols 3-5, rows 0-2    (entered via LASER at (4,3))
    // Divider col 2 (door at (2,4)) gates ROOM1->ROOM2; divider row 3 across
    // cols 3-5 (laser at (4,3)) gates ROOM2->ROOM3. Goal at (5,0). Enemies:
    // stationary in the item room (1,1); patrol in the goal room (3,1, patrols
    // col 3) — neither sits on the goal.
    mapData = [
        [0,0,1,0,0,11],   // r0  ROOM1(c0-1) | wall(c2) | ROOM3(c3-5): goal(5,0)
        [0,3,1,13,0,0],   // r1  stationary(1,1); patrol(3,1)
        [0,0,1,0,0,0],    // r2  ROOM3 bottom row
        [0,0,1,1,10,1],   // r3  divider for right side: LASER(4,3)
        [0,0,2,0,0,0],    // r4  BLUE DOOR(2,4); ROOM2(c3-5) top row
        [0,0,1,0,0,0],    // r5  spawn(0,5); ROOM2 bottom row
    ];
    startCell = { c: 0, r: 5 };

    // Pickup spots: one per tool role, randomized per seed across ROOM1
    // (cols 0-1, rows 0-5). Excludes the spawn cell + its Chebyshev-1 ring and
    // any non-floor cell (the stationary enemy). That leaves 7 candidates for 6
    // items, so the visual->cell assignment varies per seed. No value items.
    const candidates = [];
    for (let r = 0; r <= 5; r++) {
        for (let c = 0; c <= 1; c++) {
            if (Math.abs(c - startCell.c) <= 1 && Math.abs(r - startCell.r) <= 1) continue;
            if (mapData[r][c] !== 0) continue;  // skip enemy/non-floor cells
            candidates.push({ c, r });
        }
    }
    ...
    // Enemy markers -> enemy objects. tile 13 = patrolling (moves vertically),
    // tile 3 = stationary. Both lethal on contact unless a SWORD is held.
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            const t = mapData[r][c];
            if (t === 13 || t === 3) {
                enemies.push({ c, r, id: `${c}_${r}`, direction: 1, patrol: t === 13 });
                mapData[r][c] = 0;
            }
        }
    }
...
function updateGame() {
    ...
    // is slower and more predictable than the player. Each patrol enemy moves
    // 1 cell HORIZONTALLY (left<->right) in its current direction; on a
    // non-floor cell (wall/goal/edge) it reverses. If it walks into the player,
    // same collision rules as the player walking into an enemy.
    if (frameCount % PATROL_INTERVAL === 0) {
        for (let i = enemies.length - 1; i >= 0; i--) {
            const e = enemies[i];
            if (!e.patrol) continue;
            const nc = e.c + e.direction;
            const blocked = nc < 0 || nc >= COLS || mapData[e.r][nc] !== 0;
            if (blocked) {
                e.direction *= -1;
                continue;
            }
...
function tryMove(nc, nr) {
    ...
    if (here === 10) {
        // Laser (medium): always active, lethal without BOOTS, safe with BOOTS.
        // Ported from analogen_cavequest_hard.
        if (!hasItem(ROLE_BOOTS)) { die(); return; }
    }
    if (here === 4) {
        // Sunbeam: lethal without hat (no sunbeam tile on the medium map; kept for parity).
...
function tryPickup() {
    ...
    if (TOOL_VISUAL_IDS.includes(here)) {
        ...
        addItem(role, here);
        // a-cq-m-firstgrab500: +500 on the FIRST grab of this token only. Once
        // a visualId is in pickedUpTools it never pays again, so dropping and
        // re-collecting it (persistentDrops branch below, which has no reward)
        // earns nothing — no drop/re-pickup farm.
        if (!pickedUpTools.has(here)) { score += 500; pickedUpTools.add(here); }
...
function drawTile(x, y, type) {
    if (type === 1) {
        ...
        // Laser: boots visibly disable it — yellow (lethal) without boots,
        // gray (safe) with boots. Ported from analogen_cavequest_hard.
