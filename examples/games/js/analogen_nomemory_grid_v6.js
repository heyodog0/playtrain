// analogen_nomemory_grid_v6
// v4 layout with the goal moved onto the patrolling enemy's cell at (4,2)
// and v5-style BOOTS semantics (no enemy kill, separate boot sprite).
//
// Hypothesis: v4 plateaus at 6500 because the goal at (0,0) is outside
// the natural harvest path — even after solving every binding, the agent
// has no reason to walk to the far-corner. By placing the goal at (4,2),
// it sits directly on the chain (blue-door → middle-top → goal), so a
// committed harvester will physically step on it during normal play.
// Tests "does v4 stay at 6500 because exploration never finds the goal,
// or because something else prevents the win signal from propagating."
//
// Changes vs v4:
//   - Goal moved (0,0) -> (4,2). Old (0,0) becomes floor. The patrolling
...
function initRoom() {
    ...
    // v6: old goal (0,0) -> floor, patrol enemy (4,2) -> GOAL.
    mapData = [
        [0,0,1,0,1,1,0,0],
        [0,0,4,0,0,5,0,0],
        [0,0,1,0,11,1,0,0],
