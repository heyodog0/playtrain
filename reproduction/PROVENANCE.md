...
**All 87 cells match, 0 mismatches** — 29 games x three columns, checked against
the same `percmd.json` that draws `fig:envcost`, so the table and the figure
cannot disagree. Each column is a direct derivation:

| column | from `percmd.json` |
|---|---|
| cmds | sum of the game's `per_cmd` call counts |
| draw % | `draw_us / us_per_step`, as a percentage |
| steps/s | `sps` |

The job, node and dirty-rectangle setting are `fig:envcost`'s: 44381429
`envcost_adv`, `-C sapphirerapids -c 1`, anatomy measured with `QJS_DIRTY`
unset. The prose above the table states the same ("a single core of one Sapphire
Rapids node without any dirty-rectangle skipping"), and it is accurate.

Two things this table makes visible that the figure only implies:

- **`maze` at 105%.** Its priced operations exceed its measured step time, which
  is why its bar in `fig:envcost` carries no residual. The checker asserts maze
  is the only game over 100%, matching that caption.
- **The render-bound tail.** `qbert.bigmap` at 4,048 commands and 623 steps/s is
  the extreme, and it is the same 39k full-node figure quoted at L778 and in
  `tab:llm-cost` — the single-core and full-node numbers are different
  measurements of the same game and should not be confused.

Variant rows are printed under the paper's names (`breakout.multiball`,
`qbert.bigmap`, `flappy_bird.hoop`), so the checker resolves them through
`data/variant_names.tsv`, as `check_llm_cost.py` does. See § fig:learning for
why those names differ from the repo's.
