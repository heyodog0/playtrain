...
#SBATCH -t 0-01:00
#SBATCH --mem=8G
#SBATCH -J qjsweep4adv
#SBATCH -o /n/home06/truong/sweep4_adv.out
set -e
...
for g in $GAMES; do
  for t in 1 2 3 4 5 6 7; do
    ...
    echo "$g ${s:-0}" >> ~/qjs_raw4_adv.txt
  done
  echo "done qjs $g"
done
.venv-procgen/bin/python tools/bench_compare.py --backend procgen --suite procgen --games "$CSV" --frames 1500 --warmup 200 --trials 7 --fixed-actions >~/procgen_bench4_adv.log 2>&1
cp outputs/compare/procgen_procgen.json ~/procgen4_adv.json
.venv-procgen/bin/python - <<'PY'
import statistics as st, json, collections, os, math
d=collections.defaultdict(list)
for line in open(os.path.expanduser("~/qjs_raw4_adv.txt")):
    g,s=line.split(); d[g].append(float(s))
qm={g:st.fmean(v) for g,v in d.items()}
qs={g:st.stdev(v) for g,v in d.items()}
pj=json.load(open(os.path.expanduser("~/procgen4_adv.json")))["results"]
...
Qe=math.sqrt(sum((qs[g]/7**0.5)**2 for g in games))/n
