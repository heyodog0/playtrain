...
#SBATCH -t 0-00:45
#SBATCH --mem=8G
#SBATCH -J qjsatari6adv
#SBATCH -o /n/home06/truong/sweep6_adv.out
set -e
...
: > ~/qjs_atari6_adv_raw.txt
for g in $GAMES; do
  for t in 1 2 3 4 5 6 7; do
    s=$(QJS_DIRTY=1 native/build/qjs_host examples/games/js/$g.js bench 0 100000 2>/dev/null | grep -oE "[0-9]+ steps/sec" | grep -oE "^[0-9]+")
    echo "$g ${s:-0}" >> ~/qjs_atari6_adv_raw.txt
  done
  echo "done qjs $g"
done
.venv/bin/python tools/bench_compare.py --backend ale --suite atari --games "$CSV" --frames 1500 --warmup 200 --trials 7 --fixed-actions >~/ale_bench6_adv.log 2>&1
cp outputs/compare/ale_atari.json ~/ale_atari6_adv.json
.venv/bin/python - <<'PY'
import statistics as st, json, collections, os, math
d=collections.defaultdict(list)
for line in open(os.path.expanduser("~/qjs_atari6_adv_raw.txt")):
    g,s=line.split(); d[g].append(float(s))
qm={g:st.fmean(v) for g,v in d.items()}; qs={g:st.stdev(v) for g,v in d.items()}
aj=json.load(open(os.path.expanduser("~/ale_atari6_adv.json")))["results"]
...
json.dump({"qjs":qm,"qjs_std":qs,"ale":am,"ale_std":as_,"geomean_qjs":Qg,"geomean_ale":Ag,"ratio_geo":Qg/Ag},
          open(os.path.expanduser("~/compare_atari6_adv.json"),"w"),indent=1)
PY
echo SWEEP6_ADV_DONE
