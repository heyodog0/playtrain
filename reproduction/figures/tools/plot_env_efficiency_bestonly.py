...
def scaling_panel(ax, t, pg_pt, pg_epb, pg_eps, al_pt, al_epb, al_eps, fs=1.0):
    """Panel (a), documented-best-only variant: EnvPool appears once per suite,
    at its documented-best configuration (async + NUMA shards +
    thread_affinity_offset, bs-tuned; ep_best_sweep 43779854). The as-shipped
    sync1 arm is omitted here (kept in the data for the printout)."""
    for y, c, m, ls, lab in (
            (al_pt, C_PLAY_ALE, "o", "-", "PlayTrain, 8 ALE"),
            (pg_pt, C_PLAY_PG, "o", "-", "PlayTrain, 16 ProcGen"),
            (pg_epb, C_PROCGEN, "s", "--", "ProcGen (EnvPool)"),
            (al_epb, C_ALE, "s", "--", "ALE (EnvPool)")):
        ax.plot(t, y, lw=2.0 * fs, color=c, marker=m, ms=8.5 * fs, ls=ls,
                label=lab, zorder=3)
    ...
    ax.legend(loc="upper left", frameon=False, fontsize=13 * fs, handlelength=1.8,
...
def ladder_suite_panel(ax, lad, fs=1.0):
    ...
    for i, (roster, label, colour) in enumerate(suites):
        ...
        # value labels: without them the browser and Node bars are unreadable
        # next to QuickJS. Placed above each bar's whisker.
        for b, v, e in zip(bars, vals, hi):
            lab = f"{v/1000:.0f}k" if v >= 10_000 else (f"{v/1000:.1f}k" if v >= 1000 else f"{v:.0f}")
            ax.text(b.get_x() + b.get_width() / 2, v + e + cap_pad, lab,
                    ha="center", va="bottom", fontsize=11 * fs, color=INK)
    ax.set_xticks(x)
    ax.set_xticklabels([lab for _, lab in stages], fontsize=13.5)
    cap_pad = max(top) * 0.02
    for t in ax.texts:
        t.set_y(t.get_position()[1] + cap_pad)
...
def main():
    ...
    pg_epb = [177338, 354686, 584458, 777384, 931467, 1197708, 1412903]
    pg_eps = [178217, 178217, 278042, 347574, 417106, 443051, 468997]
    al_pt  = [460983, 920486, 1846763, 2760813, 3692641, 5517172, 7300384]
    al_epb = [22806, 45600, 89270, 134145, 177666, 265505, 350959]
    al_eps = [38478, 38478, 73884, 107652, 141421, 190125, 238829]
