...
Saves:
  cqhe_training_nohd_100M.png      - small multiples, one panel per N, 4 seed lines
  cqhe_training_nohd_100M_agg.png  - single panel, per-N median (EMA) + seed band
...
WIN = 55000
STEPS_M = 100        # x-axis max, millions (was 40 for the original)
...
fig.suptitle("cavequest_hard_explore (no holdout): training return per N (EMA-smoothed) — 100M steps\n"
             "dashed = win (+55k); vec recipe batch 64 @ 2e-4",
             fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.94])
out1 = "outputs/figs/cqhe_training_nohd_100M.png"
...
ax.set_title("cavequest_hard_explore (no holdout): per-N median training return — 100M steps\n"
...
fig2.tight_layout()
out2 = "outputs/figs/cqhe_training_nohd_100M_agg.png"
