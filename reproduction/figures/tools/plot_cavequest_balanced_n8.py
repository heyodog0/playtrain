"""Balanced finesweep at n=8 draws/N, across the 4 visual conditions."""
import glob, re, json
...
def ci(v):
    ...
    m=a[RNG.integers(0,len(a),size=(1000,len(a)))].mean(1); return np.percentile(m,2.5),np.percentile(m,97.5)
def curve(fn):
    d=defaultdict(list)
...
fig.tight_layout(); fig.savefig("outputs/figs/cavequest_balanced_n8_conditions.png",dpi=160); print("saved")
