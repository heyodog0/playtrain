...
def repo_to_paper():
    """Repo artifact name -> the name the paper prints (manifests/variant_names.tsv)."""
    m = {}
    for line in (R / "manifests" / "variant_names.tsv").read_text().splitlines():
...
def main():
    ...
    print(f"\n    internal inconsistencies: {len(bad)}")
    for b in bad:
        print(f"      {b}")
