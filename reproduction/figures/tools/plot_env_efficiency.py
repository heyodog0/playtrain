...
def scaling_panel(ax, t, pg_pt, pg_ep, al_pt, al_ep, fs=1.0):
    """Panel (a), in throughput_panels' house style."""
    for y, c, m, ls, lab in ((al_pt, C_PLAY_ALE, "o", "-", "PlayTrain, 8 ALE"),
                             (pg_pt, C_PLAY_PG, "o", "-", "PlayTrain, 16 ProcGen"),
                             (pg_ep, C_PROCGEN, "s", "--", "ProcGen (EnvPool)"),
                             (al_ep, C_ALE, "s", "--", "ALE (EnvPool)")):
...
def main():
    ...
    ap.add_argument("--ab-results", required=True)
    ap.add_argument("--pg-job", required=True)
    ap.add_argument("--ale-job", required=True)
    ...
    t, pg_pt, pg_ep = load_scaling(args.ab_results, args.pg_job, PROCGEN16)
    t2, al_pt, al_ep = load_scaling(args.ab_results, args.ale_job, ALE8)
    if t != t2:
        raise SystemExit(f"thread grids differ: {t} vs {t2}")
    ...
    scaling_panel(ax_sc, t, pg_pt, pg_ep, al_pt, al_ep, fs=hero_fs)
    ...
    print(f"(a) at {t[-1]} threads: ProcGen {pg_pt[-1]/pg_ep[-1]:.2f}x, "
          f"ALE {al_pt[-1]/al_ep[-1]:.2f}x")
