"""cavequest_hard_explore no-holdout sweep: mean episode RETURN vs N training
bindings (sampled eval on all 360 bindings), error bars across the seeds.
Each run's heldout_mean_return is the mean over 360 bindings; the point is the
mean over seeds at that N, error bar = std across seeds."""
import glob, re, json
...
def curve(fn):
    ...
    for f in glob.glob(f"outputs/impala_cqhe_nohd_N*_s*/{fn}"):
        ...
        d[N].append(json.load(open(f))["heldout_mean_return"])
    return d
...
for lab, (d, col) in data.items():
    ...
    print(f"{lab:11}", {n: (round(float(np.mean(d[n]))), f"±{round(float(np.std(d[n])))}") for n in sorted(d)}, "n=", ns)
...
ax.set_ylabel("Mean episode return (all 360 bindings)", fontsize=15)
ax.set_title("cavequest_hard_explore: held-out RETURN vs N training bindings\n"
             "(no holdout, SAMPLED eval on all 360; win ≈ +55k; error bars = std across seeds)", fontsize=13)
ax.legend(loc="upper left", fontsize=13, framealpha=.95)
fig.tight_layout(); fig.savefig("outputs/figs/cqhe_return_nohd.png", dpi=160); print("saved")
