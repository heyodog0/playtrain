"""Two-panel human figure with the x-axis in ENV STEPS.

Same data and binning as plot_wallclock5.py; only the x mapping and the
axis labels differ (bin centre in millions of steps instead of bin centre
divided by each run's measured sps).

(A) agent training vs 100 s of human play, per game.
(B) how much participants differ, each score over that game's median.

usage: python plot_steps.py CURVES.json STUDY_DATA_DIR OUT_DIR
"""
import glob
import json
import math
...
def band(runs):
    """Bin log-spaced in steps, not linearly.

    Linear bins over a 100M budget are 1.43M steps wide, so the first bin
    centre lands at 0.71M steps -- 9 s of PPO wall-clock -- and every point
    below that collapses into it. On a log x-axis that reads as a curve which
    starts late, when both trainers in fact log from ~100k steps onward.
    """
    # sps is no longer on the axis (x is env steps); kept for reference only
    sps = st.median([r['sps'] for r in runs if r.get('sps')] or [SPS['impala']])
    top = max(r['step'][-1] for r in runs)
    base = min(min(s_ for s_ in r['step'] if s_ > 0) for r in runs)
    span = math.log(top / base)
    edge = lambda i: base * math.exp(span * i / NBINS)
    per = []
    for r in runs:
        ...
        for s_, v in zip(r['step'], r['value']):
            if s_ <= 0:
                continue
            k = int(math.log(s_ / base) / span * NBINS)
            acc[min(max(k, 0), NBINS - 1)].append(v)
        vals, last = [], None
        for bk in acc:
            if bk:
                last = sum(bk) / len(bk)
    ...
    for i in range(NBINS):
        ...
        if col:
            # Bin centre, EXCEPT the last bin: its geometric centre sits well
            # inside the bin, so a run that reaches 100M gets drawn ending at
            # 94-97M. How far short depends on the arm's logging cadence (PPO
            # logs from 24k steps, IMPALA from 1.6M, so PPO's wider log span
            # truncates more) which made PPO look like it stopped early when it
            # actually ran further. Anchor the final point at the real last step.
            xc = top if i == NBINS - 1 else math.sqrt(edge(i) * edge(i + 1))
            xs.append(xc / 1e6)  # M steps
...
for i, g in enumerate(games):
    ...
    # linear steps axis with plain ticks, matching panel C of the learning figure
    ax.set_xlim(0, 100)
...
LOW = math.log2(0.047)   # off-scale glyph sits at the axis edge, clear of any whisker
# Box statistics are computed in LOG2 space and drawn on a linear axis whose ticks
# are labelled as ratios. Computing them on the raw ratio and then applying a log
# scale puts the 1.5*IQR fence at Q1-1.5*IQR in ratio units, which for a wide
# spread is NEGATIVE -- no point can fall below it, so the whisker is dragged out
# to the minimum and a genuine outlier never renders as one. vvvvvv is the case
# that exposed this: fence at -0.50x, so its 0.03x participant became the whisker
# end instead of a flier.
data, zeros = [], []
for g in order:
    hv = human[g]
    med = st.median(hv)
    xs = [v / med for v in hv]
    data.append([math.log2(x) for x in xs if x > 0])
...
axb.axvline(0.0, color=C_HUMAN, lw=1.0, ls='--', zorder=1)
axb.set_xlim(math.log2(0.043), math.log2(7.5))
axb.set_xticks([math.log2(v) for v in (0.25, 1, 4)])
axb.set_xticklabels(['0.25', '1', '4'])
axb.set_xticks([math.log2(v) for v in (0.125, 0.5, 2)], minor=True)
...
fig.text(0.36, 0.035, 'env steps (M)', ha='center', fontsize=7.5)
...
for ext in ('pdf', 'png'):
    fig.savefig(f'{OUT}/fig_human_wallclock.{ext}', bbox_inches='tight',
