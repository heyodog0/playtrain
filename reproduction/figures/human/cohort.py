...
plot_human_wallclock.py selects them: drop non-participant ids (probe, playtest,
debug, ...), drop sessions started before MIN_START, drop partials. Exact ages
are NOT in the committed data -- data/study/anonymize.py replaces them with
...
def main():
    ...
    print("    ages are released in bands, so the paper's mean / SD / range are not recomputed")
    ok = (len(kept) == 20 and not missing and not extra and all(blocks[g] == 20 for g in GAMES)
          and gender == collections.Counter({"Woman": 6, "Man": 13, "Non-binary": 1}))
    print(f"    20 participants, 8 games each, 6 women / 13 men / 1 non-binary: {'match' if ok else 'DIFFERS'}")
