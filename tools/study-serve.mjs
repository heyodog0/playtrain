...
//
//   node tools/study-serve.mjs            # build, serve on :8080, save to dist/study-sessions
//   node tools/study-serve.mjs --port 3000 --sessions ~/playtests
//
// Finish a session and it lands as <participantId>-<timestamp>.json, ready for
//   node tools/verify-replay.mjs dist/study-sessions/<file>.json
...
const server = createServer((req, res) => {
  if (req.method === 'POST' && req.url === '/api/session') {
    ...
    req.on('end', () => {
      ...
      console.log(`\n    verify:  node tools/verify-replay.mjs ${file}\n`);
