"""Validate bundled (or custom) Three.js games against ProcGen-style criteria.

Mirror of tools/validate.py for the Three.js (NodeGymThreeEnv) backend.
...
def collect_trajectory(game: str, seed: int, num_steps: int) -> dict:
    env = NodeGymThreeEnv(game=game, max_steps=num_steps + 100)
    try:
...
def check_reward_terminal(game: str) -> tuple[bool, str]:
    env = NodeGymThreeEnv(game=game, max_steps=500)
    try:
...
def check_throughput(game: str) -> tuple[bool, str, float]:
    env = NodeGymThreeEnv(game=game, max_steps=BENCH_FRAMES + WARMUP_FRAMES + 100)
    try:
        env.reset(seed=SEED)
        rng = np.random.default_rng(SEED)
