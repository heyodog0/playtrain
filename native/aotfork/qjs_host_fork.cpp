// qjs_host_fork.cpp — qjs_host.cpp compiled against the ivankra QuickJS fork
// (Bellard lineage + tail-call dispatch + `qjsc -A` Futamura AOT). Standalone
// L1 harness for PLAN-engine-tier.md; the ng host/vec host are NOT touched.
//   F0 (default build):  same as qjs_host — prelude + game source via JS_Eval.
//   F1 (-DHOST_AOT):     prelude + game come in as `qjsc -A` bytecode blobs
//                        (extern prelude[]/game[]), JS_ReadObject'd right after
//                        JS_NewContext so atom indices match the compile-time
//                        context (see qjsc-hostmode.patch), then JS_EvalFunction'd
//                        AFTER the p5 bindings are installed (same order as F0).
// Kept a line-for-line copy of qjs_host.cpp otherwise; diff against it.
...
int main(int argc, char** argv) {
...
#ifdef HOST_AOT
  // Read the AOT blobs FIRST: nothing may create atoms between JS_NewContext and
  // here, or the atom fixup in JS_ReadFunctionBytecode diverges from the
  // compile-time context and the engine silently falls back to interpretation
  // ("Bytecode mismatch" on stderr — the run scripts grep for it).
  extern const uint8_t qjsc_prelude[]; extern const uint32_t qjsc_prelude_size;   // qjsc default c-ident prefix is "qjsc_"
  ...
  if (JS_IsException(aotPrelude) || JS_IsException(aotGame)) { fprintf(stderr, "AOT blob read failed\n"); return 1; }
#endif
  JSValue g = JS_GetGlobalObject(ctx);
