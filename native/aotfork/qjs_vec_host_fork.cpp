// qjs_vec_host_fork.cpp — qjs_vec_host.cpp compiled against the ivankra QuickJS
// fork, for the L1 vec-host A/B (PLAN-engine-tier.md §5 L1 step 6). Same idea as
// qjs_host_fork.cpp: F0 (default) evals prelude+game source per env; F1
// (-DHOST_AOT) JS_ReadObject's the `qjsc -A` blobs per env right after
// JS_NewContext (atom order!) and JS_EvalFunction's them after the bindings.
// Bytecode is per-env (own runtime per env) but the AOT'd C functions and the
// static bytecode arrays are shared READ-ONLY across worker threads — fine.
// AOT_GAME_FNV/AOT_GAME_LEN (build-time) guard against loading a per-game .so
// with a different game's source. Otherwise a line-for-line copy; diff it.
...
static void env_init(VecHost* H, Env& e, int idx) {
  ...
  if (JS_IsException(aotPrelude) || JS_IsException(aotGame)) { e.err = "AOT blob read failed"; e.ok = false; }
#endif
...
#ifdef HOST_AOT
  { JSValue r = JS_EvalFunction(ctx, aotPrelude);
#else
  { JSValue r = JS_Eval(ctx, PRELUDE, strlen(PRELUDE), "<prelude>", JS_EVAL_TYPE_GLOBAL);
    ...
    JS_FreeValue(ctx, r); }
#ifdef HOST_AOT
  aot_intr_capture(ctx, g, e.intr_vals, e.intr_tbl);   // after the prelude (see aot_intr_capture)
#endif
...
#ifdef HOST_AOT
  { JSValue r = JS_EvalFunction(ctx, aotGame);
#else
