"""T5 (PuzzleScript): NativeVecEnv on libtwin_vec and on libqjs_vec, same game, seeds and actions, autoreset on: identical
...
@pytest.mark.parametrize("game", ["ps_sokoban_basic", "ps_kettle", "ps_whaleworld", "ps_zenpuzzlegarden"])
def test_vec_twin_equals_vec_qjs(game):
    ...
    for lib in (str(LIB), str(QJS)):
        ...
        seq = []
        for _ in range(200):
            out = v.step(rng.integers(0, v.n_actions if hasattr(v, "n_actions") else 6, size=8))
