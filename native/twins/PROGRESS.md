...
| U11 human handoff | handoff | | | FOR THE HUMAN. (1) Default `lib_path`: should `NativeVecEnv` pick `native/twins/build/libtwin_vec` for `chip8_*`/`vgdl_*`/`ps_*` bundles (a runtime change in src/playtrain)? Facts: T5 shows identical obs/reward/term/trunc to libqjs_vec on 12 games x 8 envs x 200 steps; 10-25x per env. (2) PuzzleScript observation default for training: pixel (64x64 rgb) or symbolic multihot (`getObservation`, dim in the sidecar); both implemented and traced against V8. (3) Where the 82 `.wasm` games live: `native/twins/build/wasm` today (untracked); putting `<name>.{wasm,js,json}` into the families' `dist/` means the site and `PLAYTRAIN_GAMES_DIR` users get them, and the families' own gates keep running on the JS bundles. (4) AOT tier (U07): rejected on the numbers (segfault on PuzzleScript; chip8 f1 2.5x is far below the twin's 23x). (5) Cluster: node-backend jobs 47370091 / 47372830 PENDING all day on serial_requeue; when they run, their numbers go under Numbers; twins should be benchmarked on the same node type (build.sh builds on Linux: `libtwin_vec.so`, needs the rasterizer .a and frozenmath_pic.a from native/build_qjs.sh). (6) Package split: `PACKAGE_SPLIT.md`, recommended after (1)-(3) land. (7) Two family bugs found, out of this loop's scope: vgdl `tests/golden.mjs` hashes the reset state only (NaN steps under --check/--write); the repo's `node_modules/playwright-core` symlink is dead. (8) Toolchain to install for CI: emsdk 6.0.9 (`EMSDK=`), playwright-core (`PLAYWRIGHT_CORE_DIR=`). |
...
## Summary (2026-09-20, loop finished)

Fourteen iterations, U00-U13 done, U11 left as the handoff above. Three C++ engines (CHIP-8, VGDL Colas + RC_RL,
PuzzleScript rule VM) behind one vec host (`libtwin_vec`) and one CLI (`twin_host`), every one accepted only bit for
bit against its JS bundle: 117 + 516 + 51 lockstep trajectories, 234 + 516 + 102 goldens, 78 + 52 + 68 V8 traces,
12 vec-equality games, 470/470 PuzzleScript reference tests, 193/193 CHIP-8 opcode vectors; 153 pytest gates green in
one run. Speed per env: 213k-305k (CHIP-8), 105k-222k (VGDL), 27k-168k (PuzzleScript) steps/s with observation readback,
against QuickJS's 8.5k-13k / 15k-79k / 1.2k-13k; 20 envs / 10 threads reach 1.65M / 1.0M / 679k. The same C++ builds
to 82 standalone `.wasm` games (156-296 KB) that PlayTrain's node runtime and play page load and that trace identical
to JS and native. Two negatives measured and recorded (PuzzleScript cache precompile, `qjsc -A`). Nothing in the JS
families, the QuickJS hosts, the rasterizer or the catalog changed; the runtime gained two approved loader branches.
Docs: `README.md`, `PACKAGE_SPLIT.md`, the status log in playtrain-internal, the memory note.
