...
function draw() {
  if (gameState === 'PLAYING') {
    ...
    if (keyIsDown(32) && shootCooldown <= 0) { // SPACE
      bullets.push({ x: player.x, y: player.y + 15, w: 6, h: 12, vx: -2, vy: 8 });
      bullets.push({ x: player.x, y: player.y + 15, w: 6, h: 12, vx: 0, vy: 8 });
      bullets.push({ x: player.x, y: player.y + 15, w: 6, h: 12, vx: 2, vy: 8 });
      shootCooldown = 15;
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
      ...
      if (b.y > height || b.x < -20 || b.x > width + 20) {
        bullets.splice(i, 1);
        continue;
      }
  ...
  for (let b of bullets) {
    ...
    let topX = b.x + Math.sin(angle) * (b.h / 2);
    let topY = b.y - Math.cos(angle) * (b.h / 2);

    stroke(255, 255, 255, 150);
    strokeWeight(1);
    line(player.x + 10, player.y - 15, topX, topY);
