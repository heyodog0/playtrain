...
def scaling_panel(ax, t, pg_pt, pg_epb, pg_eps, al_pt, al_epb, al_eps, fs=1.0):
    ...
    for y, c, m, ls, lab in (
            ...
            (pg_epb, C_PROCGEN, "s", "--", "ProcGen (EnvPool, documented best)"),
            (al_epb, C_ALE, "s", "--", "ALE (EnvPool, documented best)")):
...
def ladder_suite_panel(ax, lad, fs=1.0):
    ...
    for i, (roster, label, colour) in enumerate(suites):
        ...
        # value labels omitted: the adv ALE bar outgrew the published ylim and
        # its label clipped at the canvas edge
        del bars
    ax.set_xticks(x)
...
def main():
    ...
    t = [10, 20, 40, 80]
    pg_pt  = [290228, 582092, 1161420, 2325995]
    pg_epb = [355451, 587070,  931786, 1413748]
    pg_eps = [178217, 278042,  417106,  468997]
    al_pt  = [665642, 1328324, 2654832, 5308537]
